...
export interface AgentAssistantRequest extends AIRequest {
  type: 'chat_query' | 'workflow_guidance' | 'context_help' | 'quick_search' | 'auto_complete';
  data: {
    message?: string;
    current_step?: string;
    ticket_context?: any;
...
export class AgentAssistantService extends AIService {
  ...
  private async handleChatQuery(request: AgentAssistantRequest): Promise<AIResponse> {
    try {
      ...
      const session = this.getOrCreateSession(session_id, agent_id, request.context);

      const searchResponse = await this.searchRelevantInfo(message, ticket_context);
  ...
  private async provideWorkflowGuidance(request: AgentAssistantRequest): Promise<AIResponse> {
    try {
      ...
      const guidance = await this.generateWorkflowGuidance(current_step, ticket_context);
  ...
  private async provideContextHelp(request: AgentAssistantRequest): Promise<AIResponse> {
    try {
  ...
  private async performQuickSearch(request: AgentAssistantRequest): Promise<AIResponse> {
    try {
  ...
  private async provideAutoComplete(request: AgentAssistantRequest): Promise<AIResponse> {
    try {
      ...
      const suggestions = await this.generateAutoCompleteSuggestions(partial_text, ticket_context);
  ...
  private async searchRelevantInfo(query: string, context?: any): Promise<AIResponse> {
  ...
  private async generateChatResponse(
    ...
    const suggestedActions = this.generateSuggestedActions(message, ticketContext);
    const requiresEscalation = this.assessEscalationNeed(message, ticketContext);
    const confidence = this.calculateResponseConfidence(searchResults, message);

    if (sources.length === 0) {
      response = "I couldn't find specific documentation for your query. Let me suggest some general troubleshooting steps or you might want to escalate to a specialist.";
    }
  ...
  private formatContextHelp(ticketAnalysis: any, ticketContext: any): any {
    return {
      ...
      escalation_risk: {
        level: ticketAnalysis.escalation_risk > 0.7 ? 'High' :
               ticketAnalysis.escalation_risk > 0.4 ? 'Medium' : 'Low',
  ...
  private async generateAutoCompleteSuggestions(
    ...
    return commonPhrases
      .filter(phrase => phrase.toLowerCase().startsWith(partialText.toLowerCase()))
      .slice(0, 5);
  }

  private getOrCreateSession(sessionId: string, agentId: string, context?: AIContext): ChatSession {
  ...
  private assessEscalationNeed(message: string, context?: any): boolean {
    const escalationKeywords = ['critical', 'urgent', 'outage', 'production', 'escalate'];
    const lowerMessage = message.toLowerCase();
  ...
  private extractTechnologies(text: string): string[] {
    const techKeywords = ['oracle', 'postgres', 'aws', 'docker', 'java', 'python'];
    const lowerText = text.toLowerCase();

    return techKeywords.filter(tech => lowerText.includes(tech));
  }

  private determineSourceType(documentType: string): 'document' | 'ticket' | 'procedure' {
  ...
  private getBestPractices(currentStep: string): string[] {
    ...
    return practices[currentStep.toLowerCase()] || [
    ...
    setInterval(() => {
      ...
      for (const [sessionId, session] of this.chatSessions.entries()) {
        if (session.last_activity < cutoffTime) {
          ...
          logger.debug(`🧹 [AgentAssistant] Cleaned up session: ${sessionId}`);
        }
      }
