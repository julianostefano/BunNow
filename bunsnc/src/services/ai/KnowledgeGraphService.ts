...
export class KnowledgeGraphService {
  ...
  private async createIndexes(): Promise<void> {
    try {
      if (
        ...
        !this.clustersCollection ||
        !this.expertiseCollection
      ) {
      ...
      await Promise.all([
        ...
        this.expertiseCollection.createIndex({
          technology: 1,
          support_group: 1,
        }),
      ]);
  ...
  async addDocumentNode(
    ...
    try {
      ...
      console.log(
        `Knowledge graph node added: ${documentId} in ${Math.round(performance.now() - startTime)}ms`,
      );
    } catch (error: any) {
  ...
  private async processRelationships(
    ...
    for (const relationship of relationships) {
      ...
      await this.edgesCollection.replaceOne({ edge_id: edgeId }, edge, {
        upsert: true,
      });
    }
  ...
  private async createEntityNode(
    ...
    const existingNode = await this.nodesCollection.findOne({
      node_id: entityId,
    });
  ...
  private async updateConnectionCounts(): Promise<void> {
    ...
    const sourceCounts = await this.edgesCollection
      .aggregate(pipeline)
      .toArray();
  ...
  private async findRelatedDocuments(parameters: any): Promise<any> {
    ...
    const documents = await this.nodesCollection!.find(matchStage)
      .sort({ connections: -1 })
      .limit(max_results)
      .toArray();
  ...
  private async getTechnologyMap(parameters: any): Promise<TechnologyMap> {
    ...
    const technologies =
      await this.nodesCollection!.aggregate(pipeline).toArray();
  ...
  private async findKnowledgeClusters(
    ...
    const clusters = await this.clustersCollection!.find(parameters)
      .sort({ document_count: -1 })
      .toArray();
  ...
  private async getExpertiseMapping(
    ...
    const mapping = await this.expertiseCollection!.find(parameters)
  ...
  private async updateClusterAnalysis(): Promise<void> {
    ...
    for (const tech of technologies) {
      ...
      if (techNodes.length >= 3) {
        ...
        await this.clustersCollection.replaceOne({ id: cluster.id }, cluster, {
          upsert: true,
        });
      }
    }
  ...
  private async getOrphanedDocuments(): Promise<string[]> {
    ...
    const orphaned = await this.nodesCollection!.find({
      type: "document",
      connections: { $lte: 1 },
    })
      .project({ node_id: 1 })
