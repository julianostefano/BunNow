...
export interface ResolutionTimePrediction {
  ...
  similar_cases: Array<{ ticket: string; actualTime: number; accuracy: number }>;
}
...
export class PredictiveAnalyticsService {
  ...
  constructor() {
    ...
    logger.info('🔮 [PredictiveAnalytics] Service initialized with ML forecasting capabilities');
  }
  ...
  async predictTicketVolume(): Promise<TicketVolumePrediction> {
    ...
    try {
      ...
      const prediction = await this.computeVolumePrediction(historicalData, currentTrends);
  ...
  async predictResolutionTime(ticketId: string, ticketData?: any): Promise<ResolutionTimePrediction> {
    ...
    try {
      ...
      const prediction = await this.computeResolutionPrediction(ticket, similarTickets, complexityFactors);

      this.setCachedPrediction(cacheKey, prediction, 30 * 60 * 1000); // 30 min cache
      return prediction;

    } catch (error) {
      logger.error(`[PredictiveAnalytics] Resolution prediction failed for ${ticketId}:`, error);
  ...
  async assessTicketRisk(ticketId: string): Promise<RiskAssessment> {
    ...
    try {
      ...
      const mitigationSteps = await this.generateMitigationSteps(ticket, escalationRisk, breachRisk);
      ...
      logger.error(`[PredictiveAnalytics] Risk assessment failed for ${ticketId}:`, error);
  ...
  async forecastResourceNeeds(): Promise<ResourceForecast[]> {
    ...
    try {
      const supportGroups = ['Database Administration', 'Network Support', 'Application Support', 'IT Operations'];
  ...
  private async getHistoricalTicketData() {
    ...
    const response = await this.serviceNowClient.makeRequest('GET', '/incident', {
      sysparm_query: `opened_at>=${startDate.toISOString().split('T')[0]}`,
      sysparm_limit: 5000,
      sysparm_fields: 'opened_at,resolved_at,category,priority,assignment_group'
    });
  ...
  private async analyzeCurrentTrends() {
    ...
    return {
      hour: currentHour,
      day: currentDay,
      isBusinessHours: currentHour >= 9 && currentHour <= 17 && currentDay >= 1 && currentDay <= 5,
      seasonality: this.getSeasonalityFactor(now)
    };
  }

  private async computeVolumePrediction(historicalData: any[], trends: any): Promise<TicketVolumePrediction> {
  ...
  private async getTicketDetails(ticketId: string) {
    const response = await this.serviceNowClient.makeRequest('GET', '/incident', {
      ...
      sysparm_limit: 1,
      sysparm_fields: 'number,short_description,description,category,priority,assignment_group,opened_at,state'
    });
  ...
  private async findSimilarResolvedTickets(ticket: any) {
    ...
    return similarTickets.map((t: any) => ({
      ticket: t.number,
  ...
  private async analyzeTicketComplexity(ticket: any) {
    try {
      ...
      return { technical_complexity: 5, business_impact: 5, resource_requirements: 5, urgency: 5 };
    }
  }

  private async computeResolutionPrediction(ticket: any, similarTickets: any[], complexity: any): Promise<ResolutionTimePrediction> {
    const avgSimilarTime = similarTickets.length > 0
      ? similarTickets.reduce((sum, t) => sum + t.actualTime, 0) / similarTickets.length
      : 120; // Default 2 hours

    // Adjust based on complexity factors
    const complexityMultiplier = (complexity.technical_complexity + complexity.resource_requirements) / 10;
    ...
    return {
      ...
      factors: [
        ...
        { factor: 'Technical complexity', impact: complexity.technical_complexity > 6 ? 'negative' : 'positive', weight: 0.3 },
        { factor: 'Resource availability', impact: 'neutral', weight: 0.2 },
        { factor: 'Business priority', impact: ticket.priority === '1' ? 'positive' : 'neutral', weight: 0.1 }
      ],
  ...
  private async calculateEscalationRisk(ticket: any): Promise<number> {
    ...
    if (ticket.description && ticket.description.toLowerCase().includes('urgent')) risk += 0.3;
  ...
  private async calculateSLABreachRisk(ticket: any): Promise<number> {
    ...
    const slaHours = ticket.priority === '1' ? 4 : ticket.priority === '2' ? 8 : 24;
  ...
  private async generateMitigationSteps(ticket: any, escalationRisk: number, breachRisk: number): Promise<string[]> {
    const steps: string[] = [];
  ...
  private identifyUrgencyFactors(ticket: any): string[] {
    ...
    if (ticket.assignment_group === 'Database Administration') factors.push('Specialized team required');

    return factors;
  }

  private async generateGroupForecast(group: string): Promise<ResourceForecast> {
    ...
    return {
      ...
      costImpact: recommended > baseStaffing ? 'Moderate increase' : 'No change'
    };
  }
  ...
  private calculateHourlyAverages(historicalData: any[]): number[] {
    const hourlyBuckets = new Array(24).fill(0).map(() => ({ count: 0, total: 0 }));
  ...
  private calculateResolutionMinutes(openedAt: string, resolvedAt: string): number {
  ...
  private setCachedPrediction(key: string, data: any, ttl: number = this.CACHE_TTL): void {
  ...
  private getDefaultResolutionPrediction(ticketId: string): ResolutionTimePrediction {
    return {
      ticketId,
