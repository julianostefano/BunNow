...
export class TicketIntelligenceService extends AIService {
  ...
  private async analyzeTicket(
    request: TicketIntelligenceRequest,
  ): Promise<AIResponse> {
    try {
      ...
      if (ticket_id && !ticket_data) {
        try {
          const response = await this.serviceNowService.makeRequest(
            "incident",
            "GET",
            { sys_id: ticket_id },
          );
      ...
      const estimatedTime = await this.estimateResolutionTimeInternal(
        ticketInfo,
        classification,
        similarTickets,
      );
      const escalationRisk = await this.assessEscalationRiskInternal(
        ticketInfo,
        classification,
      );
  ...
  private async findSimilarTickets(
    ...
    try {
      const { ticket_id, ticket_data, description, short_description } =
        request.data;
      ...
      if (!ticketInfo && !description && !short_description) {
        return {
          success: false,
          error:
            "Ticket information, description, or short description is required",
        };
      }

      const searchText =
        ticketInfo?.short_description ||
        ticketInfo?.description ||
        short_description ||
        description ||
        "";

      const similarTickets = await this.findSimilarTicketsInternal(
        ticketInfo || {
  ...
  private async suggestResolution(
    request: TicketIntelligenceRequest,
  ): Promise<AIResponse> {
    try {
  ...
  private async assessEscalationRisk(
    request: TicketIntelligenceRequest,
  ): Promise<AIResponse> {
    try {
      const { ticket_data } = request.data;
  ...
  private async findSimilarTicketsInternal(
    ticketData: any,
  ...
  private determineCategory(text: string, ticketData: any): string {
    ...
    if (lowerText.includes("database") || lowerText.includes("sql"))
      return "Database";
    if (lowerText.includes("application") || lowerText.includes("software"))
      return "Software";
    if (lowerText.includes("hardware") || lowerText.includes("server"))
      return "Hardware";
  ...
  private determineSeverity(
    ...
    if (ticketData.severity) {
      ...
      if (severity.includes("1") || severity.toLowerCase().includes("critical"))
        return "Critical";
      if (severity.includes("2") || severity.toLowerCase().includes("high"))
        return "High";
      if (severity.includes("3") || severity.toLowerCase().includes("medium"))
        return "Medium";
    }
  ...
  private determineSourceType(
    documentType: string,
  ): "knowledge_base" | "similar_ticket" | "procedure" {
