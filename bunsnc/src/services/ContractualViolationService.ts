...
export class ContractualViolationService {
  ...
  constructor(
    ...
    this.violationTrackingCollection = this.db.collection('sn_violation_tracking');
  ...
  public async initialize(): Promise<void> {
    try {
      ...
      await this.violationTrackingCollection.createIndex({ ticket_sys_id: 1 }, { unique: true });
      ...
      logger.info(' [ContractualViolationService] Service initialized successfully');
    } catch (error) {
      logger.error(' [ContractualViolationService] Failed to initialize:', error);
      throw error;
    }
  }
  ...
  public async validateContractualViolation(
    ...
    try {
      logger.info(` [ContractualViolationService] Validating violation for ${ticketType} ${ticketId}`);

      // Get ticket data from appropriate collection
      const ticketData = await this.getTicketData(ticketId, ticketType);
      if (!ticketData) {
        throw new Error(`Ticket ${ticketId} not found in ${ticketType} collection`);
      }
      ...
      if (rules.validate_group_closure) {
        ...
        violationReasons.push({
          ...
          severity: groupValidation.closed_by_group ? ViolationSeverity.LOW : ViolationSeverity.HIGH
        });
      }
      ...
      if (rules.validate_sla_breach) {
        ...
        violationReasons.push({
          ...
          severity: slaValidation.has_sla_breach ? ViolationSeverity.CRITICAL : ViolationSeverity.LOW
        });
      }

      // Rule 3: Validate violation marking
      if (rules.validate_violation_marking) {
        const markingValidation = await this.validateViolationMarking(ticketData);
        violationReasons.push({
          ...
          severity: markingValidation.is_marked_as_violated ? ViolationSeverity.LOW : ViolationSeverity.MEDIUM
        });
      }
      ...
      logger.info(` [ContractualViolationService] Validation completed for ${ticketId}: ${isViolated ? 'VIOLATED' : 'COMPLIANT'}`);
      return result;

    } catch (error) {
      logger.error(` [ContractualViolationService] Error validating ${ticketId}:`, error);
      throw error;
    }
  }
   ...
   */
  private async validateGroupClosure(ticketData: TicketViolationData): Promise<TicketClosureData> {
    try {
      ...
      return {
        ...
        assignment_group_name: groupData?.nome || ticketData.assignment_group.display_value,
        closure_timestamp: ticketData.closed_at,
        closure_state: ticketData.state
      };

    } catch (error) {
      logger.error(' [ContractualViolationService] Error validating group closure:', error);
   ...
   */
  private async validateSLABreach(ticketData: TicketViolationData): Promise<SLABreachData> {
    try {
      const priority = this.mapPriority(ticketData.priority || '');
      const createdAt = new Date(ticketData.sys_created_on);
      const resolvedAt = ticketData.resolved_at ? new Date(ticketData.resolved_at) : null;
      ...
      if (responseSLA) {
        const responseTime = this.businessHoursCalculator.calculateBusinessHours(
          createdAt,
      ...
      if (resolutionSLA && resolvedAt) {
        const resolutionTime = this.businessHoursCalculator.calculateBusinessHours(
          createdAt,
          resolvedAt
        );
      ...
      logger.error(' [ContractualViolationService] Error validating SLA breach:', error);
      return { has_sla_breach: false, response_time_breach: false, resolution_time_breach: false, breach_details: {} };
    }
  }
   ...
   * Rule 3: Validate violation marking
   */
  private async validateViolationMarking(ticketData: TicketViolationData): Promise<ViolationMarkingData> {
    try {
      ...
      return {
        ...
        marking_timestamp: isMarked ? new Date(ticketData.sys_updated_on) : undefined
      };

    } catch (error) {
      logger.error(' [ContractualViolationService] Error validating violation marking:', error);
   ...
   * Get ticket data from appropriate collection
   */
  private async getTicketData(ticketId: string, ticketType: TicketType): Promise<TicketViolationData | null> {
    try {
      ...
      return {
        ...
        assignment_group: ticketData.assignment_group || { value: '', display_value: '' },
      ...
      logger.error(` [ContractualViolationService] Error getting ticket data for ${ticketId}:`, error);
      return null;
    }
  }
  ...
  private async storeViolationResult(
    ...
    try {
      ...
      logger.error(' [ContractualViolationService] Error storing violation result:', error);
    }
  }
  ...
  private async refreshSupportGroupsCache(): Promise<void> {
    try {
      ...
      logger.info(` [ContractualViolationService] Support groups cache refreshed: ${groups.length} groups loaded`);

    } catch (error) {
      logger.error(' [ContractualViolationService] Error refreshing support groups cache:', error);
    }
  }
  ...
  private async refreshSupportGroupsCacheIfNeeded(): Promise<void> {
    const now = new Date();
    const cacheAgeMinutes = (now.getTime() - this.cacheLastRefresh.getTime()) / (1000 * 60);
  ...
  public async generateViolationStatistics(
    ...
    try {
      ...
      const violatedTickets = violations.filter(v => v.violation_result.is_violated);

      const stats: ViolationStatistics = {
        total_tickets_analyzed: violations.length,
        total_violations_found: violatedTickets.length,
        violation_rate_percentage: violations.length > 0
          ? Math.round((violatedTickets.length / violations.length) * 10000) / 100
          : 0,
        violations_by_ticket_type: {} as Record<TicketType, number>,
        violations_by_severity: {} as Record<ViolationSeverity, number>,
        violations_by_group: {},
        total_financial_impact: violatedTickets.reduce((sum, v) => sum + (v.violation_result.financial_impact || 0), 0),
      ...
      for (const violation of violatedTickets) {
        ...
        stats.violations_by_ticket_type[ticketType] = (stats.violations_by_ticket_type[ticketType] || 0) + 1;
      }
      ...
      for (const violation of violatedTickets) {
        for (const reason of violation.violation_result.violation_reasons) {
          if (!reason.is_compliant) {
            stats.violations_by_severity[reason.severity] = (stats.violations_by_severity[reason.severity] || 0) + 1;
          }
        }
      }
      ...
      logger.error(' [ContractualViolationService] Error generating violation statistics:', error);
      throw error;
    }
  }
  ...
  public async healthCheck(): Promise<boolean> {
    try {
      ...
      logger.error(' [ContractualViolationService] Health check failed:', error);
      return false;
    }
  }
