...
export class TicketPersistenceService extends TicketStorageCore {
  ...
  async upsertTicket(
    ...
    try {
      ...
      if (slmData.length > 0) {
        normalizedTicket.sla_summary = this.generateSLASummary(
          ticketData.number,
          slmData,
        );
      }
      ...
      if (existingTicket) {
        ...
        if (
          existingTicket._hash === currentHash &&
          existingTicket._slmsHash === currentSlmsHash
        ) {
    ...
    switch (ticketType) {
      case "incident":
        return {
          ...baseTicket,
          ticketType: "incident",
          incident_state: ticketData.incident_state
            ? parseInt(ticketData.incident_state)
            : null,
          ...
          business_duration: ticketData.business_duration
            ? parseInt(ticketData.business_duration)
            : null,
          calendar_duration: ticketData.calendar_duration
            ? parseInt(ticketData.calendar_duration)
            : null,
        } as IncidentDocument;
      ...
      case "change_task":
        return {
          ...
          planned_start_date: ticketData.planned_start_date
            ? new Date(ticketData.planned_start_date)
            : null,
          planned_end_date: ticketData.planned_end_date
            ? new Date(ticketData.planned_end_date)
            : null,
          actual_start_date: ticketData.actual_start_date
            ? new Date(ticketData.actual_start_date)
            : null,
          actual_end_date: ticketData.actual_end_date
            ? new Date(ticketData.actual_end_date)
            : null,
          ...
          change_type: ticketData.change_type
            ? parseInt(ticketData.change_type)
            : null,
          risk: ticketData.risk ? parseInt(ticketData.risk) : null,
          cab_required:
            ticketData.cab_required === "true" ||
            ticketData.cab_required === true,
  ...
  private calculateSLMHash(slmData: SLMRecord[]): string {
    ...
    const sortedSLMs = [...slmData].sort((a, b) =>
      a.sys_id.localeCompare(b.sys_id),
    );
   ...
   */
  private generateSLASummary(
    ticketNumber: string,
    slmData: SLMRecord[],
  ): TicketSLASummary {
    const breachedSLAs = slmData.filter((slm) =>
      this.parseBoolean(slm.taskslatable_has_breached),
    );
    const activeSLAs = slmData.filter(
      (slm) => slm.taskslatable_stage && slm.taskslatable_stage !== "completed",
    );
      ...
      breachedSLAs
        .map((slm) => ({
          ...
          business_percentage: this.parsePercentage(
            slm.taskslatable_business_percentage,
          ),
    ...
    const allSLAs = slmData.map((slm) => ({
      ...
      end_time: slm.taskslatable_end_time,
      stage: slm.taskslatable_stage || "unknown",
      breach_time: this.parseBoolean(slm.taskslatable_has_breached)
        ? slm.taskslatable_end_time
        : undefined,
    }));
    ...
    return {
      ...
      breach_percentage:
        slmData.length > 0 ? (breachedSLAs.length / slmData.length) * 100 : 0,
      worst_sla: worstSLA,
      all_slas: allSLAs,
    };
  }
  ...
  private async recordAuditTrail(
    oldTicket: TicketDocument,
    newTicket: TicketDocument,
  ): Promise<void> {
    if (!this.db) return;
  ...
  async bulkUpsertTickets(
    ...
    for (let i = 0; i < tickets.length; i += batchSize) {
      ...
      const promises = batch.map(
        async ({ ticketData, ticketType, slmData = [] }) => {
          try {
            const result = await this.upsertTicket(
              ticketData,
              ticketType,
              slmData,
            );
