...
if (existsSync(envPath)) {
  try {
    ...
    console.log(`   Loaded vars: CORPORATE_PROXY_USER=${process.env.CORPORATE_PROXY_USER ? '***' : 'NOT_SET'}`);
    console.log(`   Loaded vars: CORPORATE_PROXY_PASSWORD=${process.env.CORPORATE_PROXY_PASSWORD ? '***' : 'NOT_SET'}`);
  } catch (error) {
