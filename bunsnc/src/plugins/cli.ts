...
export const cliPlugin = new Elysia({
      ...
      try {
        ...
        if (!user || !pass) {
          return {
            success: false,
            message:
              "Username and password required. Set SERVICENOW_USERNAME and SERVICENOW_PASSWORD environment variables.",
          };
        }

        // Use Bridge Service for authentication via ServiceNow Bridge
        console.log(
          "🔌 CLI Plugin: Testing authentication via Bridge Service...",
        );
        ...
        try {
          ...
          if (healthResponse.success && healthResponse.result?.auth) {
            console.log(
              "✅ CLI Plugin: ServiceNow authentication successful via Bridge Service",
            );
            return {
              success: true,
              message:
                "ServiceNow authentication successful via Bridge Service. SAML session established.",
            };
          } else {
            return {
              success: false,
              message:
                "ServiceNow authentication failed. Invalid credentials or network issue.",
            };
          }
        } catch (error: any) {
        ...
        console.error(
          "❌ CLI Plugin: Outer authentication error:",
          outerError.message,
        );
        return {
          success: false,
  ...
  .decorate("cliListGroups", async (): Promise<any[]> => {
    try {
      ...
      console.log(
        `✅ CLI Plugin: Retrieved ${groups.length} groups from MongoDB`,
      );
  ...
  .decorate(
    ...
    async (
      ...
      try {
        console.log(
          `🎫 CLI Plugin: Fetching real tickets from MongoDB (group: ${groupName}, state: ${state}, limit: ${limit})...`,
        );
        ...
        for (const { name: collectionName, type } of collections) {
          try {
            const collection =
              mongoCollectionManager.getCollection(collectionName);
            ...
            if (groupName) {
              filter["data.assignment_group.display_value"] = {
                $regex: groupName,
                $options: "i",
              };
            }
            ...
            const ticketsCursor = collection
              .find(filter)
            ...
            const formattedTickets = tickets.map((ticket: any) => ({
              ...
              assignment_group:
                ticket.data?.assignment_group?.display_value ||
                ticket.data?.assignment_group,
            ...
            console.log(
              `✅ Found ${tickets.length} tickets in ${collectionName}`,
            );
          } catch (collectionError: any) {
            console.warn(
              `⚠️ Error querying ${collectionName}:`,
        ...
        const sortedTickets = allTickets
          .sort(
            (a, b) =>
              new Date(b.sys_created_on).getTime() -
              new Date(a.sys_created_on).getTime(),
          )
          .slice(0, limit);

        console.log(
          `✅ CLI Plugin: Retrieved ${sortedTickets.length} total tickets from MongoDB`,
        );
  ...
  .decorate(
    ...
    ): Promise<{ success: boolean; sys_id?: string; message: string }> => {
      try {
        console.log(
          `📝 CLI Plugin: Creating real record in ServiceNow table: ${table}`,
        );
        ...
        if (bridgeResponse.success && bridgeResponse.result) {
          ...
          console.log(
            `✅ CLI Plugin: Record created successfully in ${table} via Bridge Service`,
          );
          ...
          console.log(
            `⚠️ CLI Plugin: Bridge Service creation failed for ${table}`,
          );
          return {
            success: false,
        ...
        console.error(
          `❌ CLI Plugin: Error creating record in ${table}:`,
          error.message,
        );
        return {
          success: false,
  ...
  .decorate(
    ...
    async (
      ...
      try {
        console.log(
          `✏️ CLI Plugin: Updating real record in ServiceNow table: ${table}, sys_id: ${sysId}`,
        );
        ...
        if (bridgeResponse.success && bridgeResponse.result) {
          console.log(
            `✅ CLI Plugin: Record updated successfully in ${table} via Bridge Service`,
          );
          return {
            success: true,
        ...
        console.error(
          `❌ CLI Plugin: Error updating record in ${table}:`,
          error.message,
        );
        return {
          success: false,
  ...
  .decorate(
    ...
    async (
      ...
      try {
        console.log(
          `🗑️ CLI Plugin: Deleting real record from ServiceNow table: ${table}, sys_id: ${sysId}`,
        );
        ...
        if (bridgeResponse.success) {
          console.log(
            `✅ CLI Plugin: Record deleted successfully from ${table} via Bridge Service`,
          );
          return {
            success: true,
        ...
        console.error(
          `❌ CLI Plugin: Error deleting record from ${table}:`,
          error.message,
        );
        return {
          success: false,
  ...
  .decorate(
    ...
    async (
      ...
      try {
        console.log(
          `📋 CLI Plugin: Executing real batch operations (${operations.length} operations)`,
        );

        // Use Bridge Service for batch operations
        console.log(
          `🔌 CLI Plugin: Executing ${operations.length} operations via Bridge Service...`,
        );
        ...
        for (let i = 0; i < operations.length; i++) {
          ...
          try {
            console.log(
              `📋 Processing operation ${i + 1}/${operations.length}: ${operation.op} on ${operation.table} via Bridge Service`,
            );
            ...
            switch (operation.op) {
              case "create":
                bridgeResponse = await serviceNowBridgeService.createRecord(
                  operation.table,
                  operation.data,
                );
                break;
              case "update":
                bridgeResponse = await serviceNowBridgeService.updateRecord(
                  operation.table,
                  operation.sysId,
                  operation.data,
                );
                break;
              case "delete":
                bridgeResponse = await serviceNowBridgeService.deleteRecord(
                  operation.table,
                  operation.sysId,
                );
                break;
              case "read":
              case "query":
                bridgeResponse = await serviceNowBridgeService.queryTable(
                  operation.table,
                  {
            ...
            if (bridgeResponse.success) {
              ...
              successCount++;
              console.log(
                `✅ Operation ${i + 1} successful: ${operation.op} on ${operation.table}`,
              );
            } else {
              results.push({
                index: i,
        ...
        console.error(
          "❌ CLI Plugin: Error executing batch operations:",
          error.message,
        );
        return {
          success: false,
  ...
  .decorate(
    ...
    async (
      ...
      try {
        console.log(
          `🔄 CLI Plugin: Starting real data sync for tables: ${tables?.join(", ") || "all"}`,
        );
  ...
  .decorate("getEnvVar", function (key: string, fallback: string = ""): string {
  ...
  .decorate(
    "executeCommand",
    async (command: string, args: string[]): Promise<any> => {
      try {
        console.log(
          `🚀 CLI Plugin: Executing command '${command}' with args:`,
          args,
        );
        ...
        switch (command) {
          ...
          case "create":
            return {
              message: "Use cliCreateRecord decorator to create records",
            };
          case "update":
            return {
              message: "Use cliUpdateRecord decorator to update records",
            };
          case "delete":
            return {
              message: "Use cliDeleteRecord decorator to delete records",
            };
          case "batch":
            return {
              message: "Use cliBatchOperations decorator for batch operations",
            };
          case "sync":
            return {
              message: "Use cliSyncData decorator for data synchronization",
            };
          default:
  ...
  .decorate("setupCommands", (): any => {
    ...
    return {
      ...
      message:
        "Use HTTP endpoints or direct decorator calls for CLI functionality",
    };
  })
...
export const createCLIPlugin = (config?: {
  ...
  return (app: Elysia) =>
    app.use(cliPlugin).onStart(() => {
      console.log(
        "CLI Plugin applied - command-line interface available via dependency injection",
      );
...
export type { CLICommandOptions, ServiceNowRecord };
