...
export interface StreamingPluginContext {
  ...
  subscribeToTicket: (ticketSysId: string, connectionId: string) => Promise<boolean>;
  unsubscribeFromTicket: (ticketSysId: string, connectionId: string) => Promise<boolean>;
...
export const streamingPlugin = new Elysia({
  ...
  seed: {
    ...
    unsubscribeFromTicket: {} as StreamingPluginContext['unsubscribeFromTicket'],
  ...
  .onStart(async () => {
    console.log('📡 ServiceNow Streaming Plugin starting - initializing real-time services');
  })
  ...
  .derive(async () => {
    ...
    try {
      ...
      console.warn('⚠️ Streaming Plugin: Redis Streams not available:', error.message);
    }
  ...
  .decorate('subscribeToTicket', async function(
    this: { streamingService: UnifiedStreamingService },
    ticketSysId: string,
    connectionId: string
  ): Promise<boolean> {
    try {
      await this.streamingService.subscribeToTicketUpdates(connectionId, ticketSysId);
      return true;
    } catch (error: any) {
      console.error('❌ Streaming Plugin: Error subscribing to ticket:', error.message);
      return false;
    }
  })

  .decorate('unsubscribeFromTicket', async function(
    this: { streamingService: UnifiedStreamingService },
    ticketSysId: string,
    connectionId: string
  ): Promise<boolean> {
    try {
      await this.streamingService.unsubscribeFromTicketUpdates(connectionId, ticketSysId);
      return true;
    } catch (error: any) {
      console.error('❌ Streaming Plugin: Error unsubscribing from ticket:', error.message);
      return false;
    }
  })

  // Connection management method - replaces direct service access
  .decorate('getActiveConnections', function(
    this: { streamingService: UnifiedStreamingService }
  ...
  .decorate('getStreamStats', async function(
    this: { streamingService: UnifiedStreamingService, redisStreams?: ServiceNowStreams }
  ): Promise<any> {
    try {
      const streamingStats = await this.streamingService.getStats();
      const redisStats = this.redisStreams ? await this.redisStreams.getStreamStats() : null;
      ...
      console.error('❌ Streaming Plugin: Error getting stats:', error.message);
      return {
  ...
  .decorate('sendTicketUpdate', async function(
    this: { streamingService: UnifiedStreamingService },
  ...
  .decorate('sendSLAAlert', async function(
    this: { streamingService: UnifiedStreamingService },
    slaEvent: SLAEvent
  ): Promise<void> {
  ...
  .decorate('sendSyncProgress', async function(
    this: { streamingService: UnifiedStreamingService },
  ...
  .onStop(async () => {
    console.log('🛑 ServiceNow Streaming Plugin stopping - closing all connections');
  })
   ...
   */
  .get('/streaming/sse/tickets/:ticketSysId', ({ createSSEConnection, params: { ticketSysId } }) => {
   ...
   */
  .get('/streaming/sse/sync/:table', ({ createSSEConnection, params: { table }, query }) => {
   ...
   */
  .get('/streaming/sse/sla', ({ createSSEConnection, query }) => {
   ...
   */
  .get('/streaming/sse/dashboard', ({ createSSEConnection, query }) => {
   ...
   */
  .get('/streaming/sse/general', ({ createSSEConnection, query }) => {
   ...
   */
  .get('/streaming/health', async ({ streamingService, redisStreams }) => {
    try {
      const streamingHealth = await streamingService.getHealthStatus();
      const redisHealth = redisStreams ? await redisStreams.getStreamStats() : null;
    ...
    detail: {
      ...
      description: 'Check health of streaming plugin including SSE, WebSocket, and Redis connections',
   ...
   * GET /streaming/stats
   */
  .get('/streaming/stats', async ({ getStreamStats }) => {
    try {
   ...
   */
  .get('/streaming/connections', ({ getActiveConnections }) => {
    try {
      const connections = getActiveConnections();
      const connectionData = Array.from(connections.entries()).map(([id, connection]) => ({
        id,
...
export const createStreamingPlugin = (config?: {
  ...
  return (app: Elysia) => app
    .use(streamingPlugin)
    .onStart(() => {
      console.log('🔌 Streaming Plugin applied - real-time services available via dependency injection');
      console.log('📡 SSE, WebSocket, and Redis Streams unified in single plugin');
    });
};
