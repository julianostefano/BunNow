...
import { ConsolidatedDataService, TicketData, SyncOptions, SyncResult, CacheStats, HybridDataOptions } from '../services/ConsolidatedDataService';
...
export interface DataPluginContext {
  ...
  getTicket: (sysId: string, options?: HybridDataOptions) => Promise<TicketData | null>;
  saveTicket: (ticket: TicketData, table: string) => Promise<boolean>;
  syncFromServiceNow: (table: string, options?: SyncOptions) => Promise<SyncResult>;
  getCacheStats: () => CacheStats;
  clearCache: () => void;
  warmupCache: (strategy?: any) => Promise<void>;
  getTicketsByState: (table: string, state: string, limit?: number) => Promise<TicketData[]>;
  searchTickets: (table: string, query: Record<string, any>, limit?: number) => Promise<TicketData[]>;
  batchUpdateTickets: (updates: Array<{ sysId: string; data: Partial<TicketData> }>) => Promise<number>;
}
...
export const dataPlugin = new Elysia({
  ...
  .derive(async () => {
    ...
    try {
      ...
      console.warn('⚠️ Data Plugin: Redis Streams not available:', error.message);
    }
  ...
  .decorate('getTicket', async function(
  ...
  .decorate('saveTicket', async function(
    this: { dataService: ConsolidatedDataService },
    ticket: TicketData,
    table: string
  ): Promise<boolean> {
  ...
  .decorate('syncFromServiceNow', async function(
    this: { dataService: ConsolidatedDataService },
    table: string,
    options: SyncOptions = {}
  ): Promise<SyncResult> {
    try {
      ...
      console.error('❌ Data Plugin: Error syncing from ServiceNow:', error.message);
      return {
        table,
        processed: 0,
        saved: 0,
  ...
  .decorate('getCacheStats', function(
    this: { dataService: ConsolidatedDataService }
  ): CacheStats {
    return this.dataService.getCacheStats();
  })

  .decorate('clearCache', function(
    this: { dataService: ConsolidatedDataService }
  ): void {
  ...
  .decorate('warmupCache', async function(
    this: { dataService: ConsolidatedDataService },
    strategy: any = {}
  ): Promise<void> {
    try {
  ...
  .decorate('getTicketsByState', async function(
    this: { dataService: ConsolidatedDataService },
    table: string,
    state: string,
    limit: number = 50
    ...
    try {
      ...
      console.error('❌ Data Plugin: Error getting tickets by state:', error.message);
      return [];
    }
  })
  ...
  .decorate('batchUpdateTickets', async function(
    this: { dataService: ConsolidatedDataService },
    ...
    try {
      ...
      console.error('❌ Data Plugin: Error batch updating tickets:', error.message);
      return 0;
    }
  })
  ...
  .get('/data/health', async ({ dataService, redisStreams }) => {
    ...
    detail: {
      ...
      description: 'Check health of data service plugin including MongoDB, Redis, and cache',
      tags: ['Health', 'Plugin', 'Data']
    }
  })

  // Cache metrics endpoint
  .get('/data/cache/metrics', ({ dataService }) => {
  ...
  .post('/data/cache/warmup', async ({ dataService, body }) => {
    try {
  ...
  // Sync endpoint
  .post('/data/sync/:table', async ({ dataService, params, body }) => {
    try {
      const options: SyncOptions = body || {};
      const result = await dataService.syncFromServiceNow(params.table, options);
...
export const createDataPlugin = (config?: {
  ...
  return (app: Elysia) => app
    .use(dataPlugin)
    .onStart(() => {
      console.log('🔌 Data Plugin applied - data services available via dependency injection');
      console.log('📦 Direct data service imports eliminated - using plugin injection');
    });
};

// Export types for other modules
export type { TicketData, SyncOptions, SyncResult, CacheStats, HybridDataOptions };