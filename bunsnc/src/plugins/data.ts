...
export const dataPlugin = new Elysia({
  ...
  .decorate("getTicket", async (sysId: string, options: HybridDataOptions = {}): Promise<TicketData | null> => {
    try {
      console.log(`🔍 Data Plugin: Fetching real ticket with sys_id: ${sysId}...`);
      ...
      if (ticket) {
        console.log(`✅ Data Plugin: Retrieved ticket ${ticket.number} from ${ticket.table}`);
      } else {
  ...
  .decorate("saveTicket", async (ticket: TicketData, table: string): Promise<boolean> => {
    try {
      ...
      if (!ticket || (!ticket.sys_id && !ticket.number)) {
        console.warn("⚠️ Data Plugin: Invalid ticket data - missing sys_id and number");
        return false;
      }

      console.log(`💾 Data Plugin: Saving real ticket to MongoDB: ${ticket.number || ticket.sys_id}...`);
      ...
      switch (table) {
        ...
        case "sctask":
          ...
          break;
        default:
          collection = mongoCollectionManager.getCollection(COLLECTION_NAMES.INCIDENTS);
      }
      ...
      const document = {
        ...
        sys_id_prefix: ticket.sys_id?.substring(0, 8) || ticket.number?.substring(0, 8) || 'unknown' // Safe partitioning
      };
      ...
      console.log(`✅ Data Plugin: Ticket ${success ? 'saved' : 'save failed'} in MongoDB`);
      return success;
    } catch (error: any) {
  ...
  .decorate("syncFromServiceNow", async (table: string, options: SyncOptions = {}): Promise<SyncResult> => {
    ...
    try {
      console.log(`🔄 Data Plugin: Starting real sync from ServiceNow for table: ${table} via Bridge Service...`);
      ...
      try {
        ...
        console.log(`📊 Data Plugin: Retrieved ${processed} records from ServiceNow ${table} via Bridge Service`);
        ...
        for (const ticket of tickets) {
          try {
            ...
            const saveResult = await dataPlugin.decorator.saveTicket(ticketData, table);
      ...
      console.log(`✅ Data Plugin: Sync completed for ${table}: ${saved} saved, ${updated} updated, ${errors} errors in ${duration}ms`);
      return result;
    } catch (error: any) {
      console.error(`❌ Data Plugin: Sync failed for ${table}:`, error.message);
  ...
  .decorate("getCacheStats", (): CacheStats => {
    try {
      ...
      console.warn("⚠️ Data Plugin: Could not get real cache stats, using defaults:", error.message);
      return {
        hitRatio: 0,
        missRatio: 0,
        totalRequests: 0,
  ...
  .decorate("clearCache", (): void => {
    try {
      ...
      console.warn("⚠️ Data Plugin: Could not clear real cache:", error.message);
    }
  })

  .decorate("warmupCache", async (strategy: any = {}): Promise<void> => {
    try {
      console.log("🔥 Data Plugin: Starting real cache warmup via Bridge Service...");
      ...
      console.log(`📊 Data Plugin: Warming up cache for ${tables.length} tables via Bridge Service...`);
      ...
      for (const table of tables) {
        try {
          ...
          console.log(`✅ Data Plugin: Cache warmed for ${table} via Bridge Service`);
        } catch (tableError: any) {
          console.warn(`⚠️ Data Plugin: Cache warmup failed for ${table}:`, tableError.message);
        }
      }
  ...
  .decorate("getTicketsByState", async (table: string, state: string, limit: number = 50): Promise<TicketData[]> => {
    try {
      console.log(`🔍 Data Plugin: Querying real tickets from ${table} with state: ${state}, limit: ${limit}...`);
      ...
      console.log(`✅ Data Plugin: Retrieved ${tickets.length} tickets from ${table} with state ${state}`);
      return tickets;
    } catch (error: any) {
      console.error(`❌ Data Plugin: Error querying tickets by state:`, error.message);
      return [];
    }
  })
  ...
  .decorate("searchTickets", async (table: string, query: Record<string, any>, limit: number = 50): Promise<TicketData[]> => {
    try {
      console.log(`🔎 Data Plugin: Searching real tickets in ${table} with query:`, query);
      ...
      Object.entries(query).forEach(([key, value]) => {
        if (key === "sys_id") {
          ...
          if (typeof value === "string") {
            mongoQuery["$or"] = [
              { "data.assignment_group.display_value": { $regex: value, $options: "i" } },
      ...
      console.log(`✅ Data Plugin: Found ${tickets.length} tickets matching search criteria`);
      return tickets;
    } catch (error: any) {
      console.error(`❌ Data Plugin: Error searching tickets:`, error.message);
      return [];
    }
  })
  ...
  .decorate("batchUpdateTickets", async (updates: Array<{ sysId: string; data: Partial<TicketData> }>): Promise<number> => {
    try {
      ...
      if (!Array.isArray(updates) || updates.length === 0) {
        console.warn("⚠️ Data Plugin: Invalid batch updates - empty or not array");
        return 0;
      }
      ...
      console.log(`📋 Data Plugin: Performing real batch update for ${validUpdates.length} valid tickets (${updates.length - validUpdates.length} filtered out)...`);
      ...
      for (const update of validUpdates) {
        try {
          ...
          const collections = [
            { name: COLLECTION_NAMES.INCIDENTS, getter: () => mongoCollectionManager.getIncidentsCollection() },
            { name: COLLECTION_NAMES.CHANGE_TASKS, getter: () => mongoCollectionManager.getChangeTasksCollection() },
            { name: COLLECTION_NAMES.SC_TASKS, getter: () => mongoCollectionManager.getSCTasksCollection() }
          ];
          ...
          for (const { name, getter } of collections) {
            try {
              ...
              const existingDoc = await collection.findOne({ sys_id: update.sysId });
              ...
              if (existingDoc) {
                ...
                const updateDoc = {
                  $set: {
                    ...
                    updated_at: new Date(),
                    ...Object.fromEntries(
                      Object.entries(update.data).map(([key, value]) => [`data.${key}`, value])
                    )
                  }
                };
                ...
                if (result.modifiedCount > 0) {
                  successCount++;
                  updated = true;
                  console.log(`✅ Data Plugin: Updated ticket ${update.sysId} in ${name}`);
                  break;
                }
              }
            } catch (collectionError: any) {
              console.warn(`⚠️ Data Plugin: Error updating in ${name}:`, collectionError.message);
            }
          }

          if (!updated) {
            console.warn(`⚠️ Data Plugin: Ticket ${update.sysId} not found in any collection`);
          }
        } catch (updateError: any) {
          console.error(`❌ Data Plugin: Error updating ticket ${update.sysId}:`, updateError.message);
        }
      }

      console.log(`✅ Data Plugin: Batch update completed: ${successCount}/${updates.length} successful`);
      return successCount;
    } catch (error: any) {
