...
import { ServiceNowAuthClient, ServiceNowRecord, ServiceNowQueryResult } from '../services/ServiceNowAuthClient';
...
export interface AuthPluginContext {
  ...
  makeAuthenticatedRequest: (table: string, method?: string, params?: Record<string, unknown>) => Promise<ServiceNowQueryResult>;
  searchTickets: (searchTerm: string, tables?: string[], limit?: number) => Promise<ServiceNowRecord[]>;
  getWaitingTickets: (groups: string[]) => Promise<any>;
  getSLAData: (taskSysId: string) => Promise<ServiceNowRecord[]>;
  getContractSLA: (company?: string, location?: string) => Promise<ServiceNowRecord[]>;
  getCacheMetrics: () => any;
}
...
export const authPlugin = new Elysia({
  ...
  seed: {
    ...
    makeAuthenticatedRequest: {} as AuthPluginContext['makeAuthenticatedRequest'],
  ...
  .decorate('makeAuthenticatedRequest', async function(
    ...
    if (!this.authClient.isAuthValid()) {
      throw new Error('Authentication required - please check ServiceNow credentials');
    }
  ...
  .get('/auth/health', async ({ authClient }) => {
    ...
    detail: {
      ...
      description: 'Check health of ServiceNow authentication plugin and cache status',
      tags: ['Health', 'Plugin', 'Auth']
    }
  })

  // Authentication status endpoint
  .get('/auth/status', ({ authClient }) => {
    const isValid = authClient.isAuthValid();
  ...
  .get('/auth/cache/metrics', ({ authClient }) => {
...
export const createAuthPlugin = (config?: {
  ...
  return (app: Elysia) => app
    .use(authPlugin)
    .onStart(() => {
      console.log('🔌 Auth Plugin applied - authentication service available via dependency injection');
      console.log('📦 Direct auth client imports eliminated - using plugin injection');
    });
};
