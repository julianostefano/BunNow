...
async function getWaitingTicketsDetails(targetGroup?: string): Promise<any[]> {
    ...
    try {
        ...
        const details = await serviceNowAuthClient.getWaitingTicketsDetails(groupsToQuery);
        return details;
...
export async function waitingTicketsCommand(options: { group?: string; details?: boolean }) {
    try {
        ...
        if (options.details) {
            ...
            if (totalWaiting === 0) {
                console.log('🎉 Excelente! Nenhum chamado em espera nos grupos de fallback!');
            } else {
                console.log('💡 Dica: Use --details para ver os chamados específicos');
                console.log('💡 Exemplo: bun src/cli/waiting-tickets.ts --details --group "L2-NE-IT NETWORK"');
            }
...
if (import.meta.main) {
    ...
    const options = {
        ...
        group: args.includes('--group') ? args[args.indexOf('--group') + 1] : undefined
    };
