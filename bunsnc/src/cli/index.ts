...
const mockBatchService = {
	executeBatch: async () => ({ result: "Batch executado com sucesso", sys_id: "mockedBatch" })
};
const mockAttachmentService = {
	upload: async () => ({ result: "Upload realizado com sucesso", sys_id: "mockedAttach" }),
...
export async function main() {
		switch (command) {
						...
						case "update": {
							...
							try {
								const result = await controller.update(args[0], args[1], JSON.parse(args[2]));
								console.log(result);
							} catch (e) {
