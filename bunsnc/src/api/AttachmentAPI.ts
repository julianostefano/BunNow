...
export interface IAttachmentAPI {
  ...
  upload(fileName: string, table: string, tableSysId: string, file: File | Buffer | Blob, contentType?: string): Promise<string>;
  ...
  getFileAsLines(sysId: string, encoding?: string, delimiter?: string): Promise<string[]>;
  saveToFile(sysId: string, filePath: string): Promise<string>;
  asTempFile(sysId: string, chunkSize?: number): Promise<Blob>;
  getWithStats(sysId: string): Promise<ServiceNowRecord & { fileExists: boolean; accessible: boolean }>;
  bulkDelete(sysIds: string[]): Promise<{ deleted: number; errors: Array<{ sysId: string; error: string }> }>;
  streamDownload(sysId: string): Promise<ReadableStream>;
  uploadWithProgress(fileName: string, table: string, tableSysId: string, file: File | Buffer | Blob, onProgress?: (progress: number) => void): Promise<string>;
...
export class AttachmentAPI implements IAttachmentAPI {
  ...
  constructor(private instanceUrl: string, private authToken: string, options: {
  ...
  async get(sysId: string): Promise<ServiceNowRecord | null> {
    const operation = logger.operation('get_attachment_metadata', undefined, sysId, {
      apiId: this.apiId
    });
  ...
  async list(table: string, tableSysId: string): Promise<ServiceNowRecord[]> {
    ...
    try {
      ...
      const params = new URLSearchParams({
        ...
        'sysparm_fields': 'sys_id,file_name,content_type,size_bytes,size_compressed,state,table_name,table_sys_id,sys_created_on,sys_created_by',
      ...
      operation.success('Attachment list retrieved', {
        ...
        totalSize: attachments.reduce((sum: number, att: any) => sum + (parseInt(att.size_bytes) || 0), 0)
      });
  ...
  async upload(
    ...
    return this.uploadWithProgress(fileName, table, tableSysId, file, undefined, contentType);
  }
  ...
  async uploadWithProgress(
    ...
    const fileSize = file instanceof Buffer ? file.length : file instanceof Blob ? file.size : 'unknown';
  ...
  async getFile(sysId: string, stream: boolean = false): Promise<Response> {
    const operation = logger.operation('get_attachment_file', undefined, sysId, {
      apiId: this.apiId,
      streaming: stream
    });
  ...
  async delete(sysId: string): Promise<boolean> {
    ...
    try {
      ...
      if (response.status === 404) {
        operation.success('Attachment not found for deletion', { found: false });
   ...
   */
  async getFileAsText(sysId: string, encoding: string = 'utf-8'): Promise<string> {
    try {
      const response = await this.getFile(sysId);
   ...
   */
  async getFileAsLines(sysId: string, encoding: string = 'utf-8', delimiter: string = '\n'): Promise<string[]> {
    try {
   ...
   */
  async getWithStats(sysId: string): Promise<ServiceNowRecord & { fileExists: boolean; accessible: boolean }> {
    try {
  ...
  async streamDownload(sysId: string): Promise<ReadableStream> {
    const operation = logger.operation('stream_download_attachment', undefined, sysId, {
      apiId: this.apiId
    });
  ...
  enableCaching(enabled: boolean): void {
    ...
    logger.info(`AttachmentAPI caching ${enabled ? 'enabled' : 'disabled'}`, 'AttachmentAPI', {
      apiId: this.apiId
    });
  }
   ...
   */
  async bulkDelete(sysIds: string[]): Promise<{ deleted: number; errors: Array<{ sysId: string; error: string }> }> {
    const operation = logger.operation('bulk_delete_attachments', undefined, undefined, {
      apiId: this.apiId,
      attachmentCount: sysIds.length
    });

    const results = { deleted: 0, errors: [] as Array<{ sysId: string; error: string }> };
