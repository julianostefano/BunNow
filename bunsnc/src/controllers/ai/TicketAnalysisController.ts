...
export class TicketAnalysisController {
  ...
  async analyzeTicket(request: TicketAnalysisRequest): Promise<TicketAnalysisResponse> {
    ...
    try {
      ...
      const predictions = await this.generatePredictions(enhancedContent, similarTickets);
      ...
      const response: TicketAnalysisResponse = {
        ...
        analysis: {
          ...
          predictions,
          recommendations,
          confidence_score: this.calculateConfidenceScore(similarTickets, predictions),
  ...
  private async enhanceTicketContent(request: TicketAnalysisRequest): Promise<any> {
    const content = {
    ...
    if (content.attachments.length > 0) {
      logger.info(`📎 [TicketAnalysis] Processing ${content.attachments.length} attachments`);
  ...
  private async findSimilarTickets(
    ...
    try {
      ...
      const combinedResults = this.combineSearchResults(semanticResults, hybridResults);

      // 4. Rerank using neural reranker
      const documents = combinedResults.map(result => ({
        id: result.sys_id,
        text: `${result.short_description} ${result.description}`.substring(0, 1000)
      }));
      ...
      for (const rerankResult of rerankResponse.results) {
        const originalTicket = combinedResults.find(t => t.sys_id === rerankResult.document.id);
        if (originalTicket) {
          similarTickets.push({
    ...
    try {
      ...
      if (this.config.enable_category_prediction) {
        const categoryPrediction = await this.predictTicketProperty(
          ...
          content,
          similarTickets,
          ['Hardware', 'Software', 'Network', 'Security', 'Database', 'Application']
        );
      ...
      if (this.config.enable_assignment_prediction) {
        const assignmentPrediction = await this.predictAssignmentGroup(content, similarTickets);
  ...
  private async predictTicketProperty(
    ...
    const propertyFrequency = new Map<string, { count: number; totalScore: number }>();
    ...
    for (const ticket of similarTickets) {
      ...
      if (value && possibleValues.includes(value)) {
        const current = propertyFrequency.get(value) || { count: 0, totalScore: 0 };
        current.count++;
    ...
    const predictions = Array.from(propertyFrequency.entries())
      .map(([value, stats]) => ({
        value,
        confidence: Math.min(0.95, (stats.totalScore / stats.count) * 0.8 + (stats.count / similarTickets.length) * 0.2),
  ...
  private async generateRecommendations(
    ...
    try {
      ...
      if (highConfidencePredictions.length > 0) {
        recommendations.push(`Recomendação de atribuição: ${highConfidencePredictions.find(p => p.property === 'assignment_group')?.predicted_value || 'Service Desk'}`);
      }
    ...
    return recommendations.length > 0 ? recommendations : ['Analisar contexto e aplicar procedimentos padrão'];
  }

  private combineSearchResults(semanticResults: any[], hybridResults: any[]): any[] {
  ...
  private calculateConfidenceScore(
    ...
    const avgSimilarity = similarTickets.reduce((sum, t) => sum + t.similarity_score, 0) / similarTickets.length;
