...
export class DocumentIntelligenceController {
  ...
  constructor() {
    try {
      ...
      if (!openSearchHost || !openSearchPort) {
        logger.warn(
          `⚠️ [DocumentIntelligenceController] OpenSearch environment variables missing. Using defaults: host=${openSearchHost || "10.219.8.210"}, port=${openSearchPort || "9200"}`,
        );
      }
  ...
  async processDocument(
    ...
    try {
      logger.info(
        `📄 [DocumentIntelligence] Processing document: ${request.filename}`,
      );
  ...
  async searchDocuments(
    ...
    try {
      logger.info(
        ` [DocumentIntelligence] Searching documents: "${request.query}"`,
      );
      ...
      const documents = searchResults.map((result) => ({
        id: result._id,
        text: `${result._source.title || ""} ${result._source.content}`.substring(
          0,
          1000,
        ),
      }));
  ...
  private validateDocument(request: DocumentProcessingRequest): void {
    ...
    if (request.content_buffer.length > this.config.max_document_size) {
      throw new Error(
        `Document size exceeds limit of ${this.config.max_document_size} bytes`,
      );
    }
  ...
  private mapNERField(field: string): string {
    ...
    if (fieldLower.includes("person") || fieldLower.includes("people"))
      return "PERSON";
    if (fieldLower.includes("organization") || fieldLower.includes("company"))
      return "ORG";
    if (fieldLower.includes("location") || fieldLower.includes("place"))
      return "LOC";
    ...
    if (fieldLower.includes("money") || fieldLower.includes("currency"))
      return "MONEY";
  ...
  private inferDocumentType(content: string, filename: string): string {
    ...
    if (["md", "txt"].includes(ext || "") && content.includes("#"))
  ...
  private chunkDocument(
    ...
    for (const paragraph of paragraphs) {
      if (
        (currentChunk + paragraph).length > this.config.chunk_size &&
        ...
        const overlapWords = Math.floor(
          this.config.chunk_overlap / (currentChunk.length / words.length),
        );
  ...
  private async extractKnowledge(
    ...
    try {
      ...
      const complexityScore = technicalIndicators.reduce(
        (score, indicator) =>
          score + (content.toLowerCase().includes(indicator) ? 1 : 0),
        0,
      );
  ...
  private buildSearchFilters(
    ...
    if (request.filters) {
      ...
      if (request.filters.technology_stack) {
        filters["classification.technology_stack"] =
          request.filters.technology_stack;
      }
    ...
    return rerankResults
      .map((rerankResult) => {
        const original = originalResults.find(
          (r) => r._id === rerankResult.document.id,
        );
  ...
  async updateConfig(
    newConfig: Partial<DocumentIntelligenceConfig>,
  ): Promise<void> {
