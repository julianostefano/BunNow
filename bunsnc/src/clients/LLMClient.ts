...
export class LLMClient {
  ...
  constructor(config?: Partial<LLMConfig>) {
    this.config = {
      ...
      default_model:
        config?.default_model || process.env.LLM_MODEL || "deepseek-coder:1.3b",
      max_tokens:
        config?.max_tokens || parseInt(process.env.LLM_MAX_TOKENS || "2048"),
      temperature:
        config?.temperature || parseFloat(process.env.LLM_TEMPERATURE || "0.7"),
    };
    ...
    logger.info(
      ` [LLMClient] Initialized with URL: ${this.baseUrl}, Model: ${this.config.default_model}`,
    );
  }
  ...
  async generateResponse(
    ...
    try {
      ...
      logger.debug(
        ` [LLMClient] Generated response with ${result.eval_count || 0} tokens in ${result.total_duration ? Math.round(result.total_duration / 1000000) : 0}ms`,
      );
  ...
  async generateStreamResponse(
    ...
    try {
      ...
      if (!response.ok) {
        const errorText = await response.text();
        throw new Error(
          `LLM streaming failed: ${response.status} ${response.statusText} - ${errorText}`,
        );
      }
        ...
        while (true) {
          ...
          for (const line of lines) {
            try {
              ...
              logger.warn(
                " [LLMClient] Failed to parse stream chunk:",
                parseError,
              );
            }
          }
  ...
  async healthCheck(): Promise<boolean> {
    try {
      ...
      if (response.ok) {
        ...
        logger.debug(
          ` [LLMClient] Health check passed - Available models: ${data.models?.length || 0}`,
        );
        return true;
      }
  ...
  async warmup(): Promise<boolean> {
    try {
      ...
      await this.generateCompletion(
        "Hello, this is a test message to warm up the model.",
        {
          max_tokens: 50,
          temperature: 0.1,
        },
