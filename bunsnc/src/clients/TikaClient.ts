...
export class TikaClient {
  ...
  constructor(tikaUrl?: string, timeout: number = 30000) {
    this.tikaUrl =
      tikaUrl || process.env.TIKA_SERVER_URL || "http://localhost:9999";
  ...
  async extractText(fileBuffer: Buffer, mimeType?: string): Promise<string> {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Tika extraction failed: ${response.status} ${response.statusText}`,
        );
      }
  ...
  async extractMetadata(
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Tika metadata extraction failed: ${response.status} ${response.statusText}`,
        );
      }
  ...
  async extractFull(
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Tika full extraction failed: ${response.status} ${response.statusText}`,
        );
      }
  ...
  async healthCheck(): Promise<boolean> {
    try {
      ...
      if (isHealthy) {
        ...
        logger.debug(
          ` [TikaClient] Health check passed - Version: ${version.trim()}`,
        );
      }
