...
export class OpenSearchClient {
  ...
  async neuralSearch(
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Neural search failed: ${response.status} ${response.statusText}`);
      }
  ...
  async rerankSearch(
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Rerank search failed: ${response.status} ${response.statusText}`);
      }
  ...
  private buildNeuralQuery(query: NeuralSearchQuery, context?: SearchContext): any {
    const baseQuery = {
      size: query.size || 10,
      query: {
        neural: {
