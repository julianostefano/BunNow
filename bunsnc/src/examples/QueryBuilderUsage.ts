...
export class QueryBuilderExamples {
  ...
  async findIncidentsAssignedToTeam() {
    ...
    const assignedCondition = queryBuilder.addQuery('assigned_to', 'CONTAINS', 'john.doe');
    assignedCondition.addOrCondition('assignment_group', 'CONTAINS', 'IT Support');
    assignedCondition.addOrCondition('assignment_group', 'CONTAINS', 'Network Team');
  ...
  async findCIWithOpenIncidents() {
    const queryBuilder = this.recordController.createQueryBuilder('cmdb_ci_computer');
    ...
    return this.recordController.queryWithBuilder('cmdb_ci_computer', queryBuilder);
  }
   ...
   */
  buildDynamicQuery(table: string, filters: { field: string; operator: string; value: any }[]) {
