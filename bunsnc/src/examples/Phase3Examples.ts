 ...
 */
import { ServiceNowClient, GlideRecord, BatchAPI, type BatchRequest } from '../bunsnc';
...
export async function glideRecordPaginationExamples(client: ServiceNowClient) {
  ...
  try {
    ...
    let count = 0;
    while (gr.next() && count < 10) { // Limit to first 10 for demo
      console.log(`  - User ${count + 1}: ${gr.getValue('name')} (${gr.getValue('email')})`);
      count++;
    }
    ...
    while (await grAsync.nextAsync() && asyncCount < 5) {
      console.log(`  - Audit ${asyncCount + 1}: ${grAsync.getValue('tablename')} - ${grAsync.getValue('operation')}`);
      asyncCount++;
    }
