...
export class GlideRecordExamples {
  ...
  async advancedQuerying() {
    ...
    try {
      ...
      while (gr.next()) {
        ...
        console.log(`  State: ${gr.getDisplayValue('state')}, Priority: ${gr.getValue('priority')}`);
  ...
  async iterationPatterns() {
    ...
    try {
      ...
      while (gr.next()) {
        count++;
        console.log(`${count}. ${gr.getValue('number')}: ${gr.getValue('short_description')}`);
  ...
  async batchOperationsExample() {
    ...
    try {
      ...
      while (gr.next()) {
        ...
        if (processed % 50 === 1) {
          batch++;
          console.log(`Processing batch ${batch} - Record: ${number} (${updatedOn})`);
        }
