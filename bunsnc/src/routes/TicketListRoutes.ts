  ...
  return new Elysia({ prefix: '/tickets' })
    ...
    .get('/lazy-load/:type/:state', async ({ params, query, set }) => {
      try {
        ...
        ErrorHandler.logError('Lazy Load Tickets', error, { type: params.type, state: params.state });
        
        const errorHtml = TicketListView.generateErrorCard('Erro ao carregar tickets');
        set.headers['content-type'] = 'text/html; charset=utf-8';
        return errorHtml;
      }
    }, {
    ...
    .get('/ticket-counts/:type/:state', async ({ params, query, serviceNowAuthClient, set }) => {
      try {
        const { type, state } = params;
        ...
        ErrorHandler.logError('Ticket Counts', error, { type: params.type, state: params.state });
