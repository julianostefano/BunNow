...
export const createModalRoutes = () => {
  return new Elysia({ prefix: '/modal' })
    ...
    .get('/ticket/:table/:sysId', async ({ params, query, set }) => {
    ...
    .get('/data/:table/:sysId', async ({ params, query }) => {
      const startTime = Date.now();
      ...
      try {
        ...
        switch (params.section) {
          ...
          case 'notes':
            ...
            sectionHtml = this.generateNotesUpdate(ticket.notes || [], ticket);
            break;
...
export const createSSERoutes = () => {
  return new Elysia({ prefix: '/events' })
    ...
    .get('/ticket-updates/:sysId', async ({ params, set }) => {
      try {
        logger.info(`📡 SSE connection established for ticket ${params.sysId}`);
        ...
        const stream = new ReadableStream({
          ...
          cancel() {
            isConnected = false;
            logger.info(`📡 SSE connection closed for ticket ${params.sysId}`);
          }
        });
