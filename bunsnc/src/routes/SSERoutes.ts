...
export const createSSERoutes = (redisStreams: ServiceNowStreams) => {
  ...
  return new Elysia({ prefix: '/sse' })
    ...
    .get('/stream/tickets/:sysId', function* ({ params: { sysId } }) {
      ...
      yield* unifiedStreamingService.createStream(clientId, 'ticket-updates', { 
        ticketSysId: sysId,
        maxHistory: 10
      });
    })
    ...
    .get('/stream/dashboard', function* ({ query }) {
      ...
      yield* unifiedStreamingService.createStream(clientId, 'dashboard-stats', { 
        intervalSeconds 
      });
    })
    ...
    .get('/stream/sla', function* ({ query }) {
      ...
      const filters = {
        ...
        ticketTypes: query.types ? (query.types as string).split(',') : undefined
      };
      yield* unifiedStreamingService.createStream(clientId, 'sla-monitoring', { 
        filters 
      });
    })
