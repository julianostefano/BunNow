  ...
  return (
    new Elysia({ prefix: "/api/groups" })
      ...
      .onError({ as: "global" }, (context) => {
        ...
        logger.error(
          `🚨 [GROUP-API-GLOBAL-ERROR] Code: ${code}, Error: ${error?.message || "Unknown"}`,
        );
        ...
        if (error?.message && error.message.includes("_r_r is not defined")) {
          logger.error(
            "🚨 [ELYSIA-BUG] Detected _r_r is not defined error - applying workaround",
          );
          const frameworkError = new ElysiaFrameworkError(
            "_r_r is not defined bug detected",
            error,
          );
        ...
        return {
          ...
          message:
            error?.message || "An unexpected error occurred in the Groups API",
          timestamp: new Date().toISOString(),
        };
      })
      ...
      .onRequest(({ request, path }) => {
        ...
        logger.info(
          ` [GROUP-API-REQUEST] ${request.method} ${path} - ${timestamp} - UA: ${userAgent.substring(0, 50)}`,
        );
      })
      ...
      .onAfterResponse(({ request, path, elapsed }) => {
        ...
        logger.info(
          ` [GROUP-API-RESPONSE] ${method} ${path} completed in ${elapsed.toFixed(2)}ms`,
        );

        // Log performance warnings
        if (elapsed > 1000) {
          logger.warn(
            ` [GROUP-API-SLOW] Slow request detected: ${method} ${path} took ${elapsed.toFixed(2)}ms`,
          );
        }
        ...
        if (elapsed < 100) {
          logger.debug(
            ` [GROUP-API-FAST] Fast request: ${method} ${path} took ${elapsed.toFixed(2)}ms`,
          );
        }
      })
      .get(
        ...
        async ({ query }) => {
          try {
            ...
            if (query.responsavel)
              filter.responsavel = String(query.responsavel);
            if (query.temperatura)
              filter.temperatura = Number(query.temperatura);
            if (query.temperaturaMin)
              filter.temperaturaMin = Number(query.temperaturaMin);
            if (query.temperaturaMax)
              filter.temperaturaMax = Number(query.temperaturaMax);
      ...
      .get("/dropdown", async () => {
        try {
          logger.info(
            "📋 [API] GET /api/groups/dropdown - Fetching dropdown options",
          );
          ...
          const dropdownOptions = allGroups.map((group) => ({
            ...
            label: group.nome,
            emoji:
              group.temperatura <= 2 ? "🔥" : group.temperatura <= 4 ? "" : "",
          }));
      ...
      .get("/stats", async () => {
        try {
          logger.info(
            " [API] GET /api/groups/stats - Fetching collection statistics",
          );
      ...
      .get(
        ...
        async ({ params }) => {
          try {
            ...
            logger.info(
              `📋 [API] GET /api/groups/${groupId} - Fetching group by ID`,
            );
      ...
      .get(
        ...
        async ({ params }) => {
          try {
            ...
            logger.info(
              `📋 [API] GET /api/groups/name/${groupName} - Fetching group by name`,
            );

            const groups = await hybridDataService.getAllGroups({
              nome: groupName,
            });
            const group =
              groups.length > 0
                ? await hybridDataService.getGroupById(groups[0].id)
                : null;
      ...
      .get(
        ...
        async ({ params }) => {
          try {
            ...
            logger.info(
              `📋 [API] GET /api/groups/tag/${tag} - Fetching groups by tag`,
            );
      ...
      .get(
        ...
        async ({ params }) => {
          try {
            ...
            logger.info(
              `📋 [API] GET /api/groups/responsavel/${responsavel} - Fetching groups by responsavel`,
            );
      ...
      .delete(
        ...
        async ({ params }) => {
          try {
            ...
            logger.info(
              `🗑️ [API] DELETE /api/groups/${groupId} - Deleting group`,
            );
