...
export function createIncidentNotesRoutes(
  ...
  return new Elysia({ prefix: '/api/incident' })
    ...
    .get('/details/:sysId', async ({ params, set }) => {
      try {
        ...
        const result = {
          ...
          notes: {
            work_notes: notesResponse?.result?.map(note => ({
            ...
            comments: commentsResponse?.result?.map(comment => ({
          ...
          sla: {
            ...
            task_sla_records: slaResponse?.result?.map(sla => ({
    ...
    .get('/notes/:sysId', async ({ params, set }) => {
      try {
        ...
        const result = {
          ...
          total_notes: (notesResponse?.result?.length || 0) + (commentsResponse?.result?.length || 0),
          retrieved_at: new Date().toISOString()
        };
    ...
    .get('/sla/:sysId', async ({ params, set }) => {
      try {
        ...
        const result = {
          sys_id: sysId,
          sla_records: slaResponse?.result?.map(sla => ({
            sys_id: extractValue(sla.sys_id),
