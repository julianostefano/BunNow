...
export class PerformanceMonitor {
  ...
  setEnabled(enabled: boolean): void {
    ...
    logger.info(`Performance monitoring ${enabled ? 'enabled' : 'disabled'}`, 'PerformanceMonitor');
  }
   ...
   * Record request rate
   */
  recordRequestRate(requests: number, timeWindow: number, context?: string): void {
    if (!this.enabled) return;
  ...
  private setupDefaultThresholds(): void {
    ...
    this.setThreshold('memory_heap_used', 100 * 1024 * 1024, 500 * 1024 * 1024, 'bytes');
  ...
  private checkThresholds(metric: PerformanceMetric): void {
    ...
    if (metric.value >= threshold.critical) {
      logger.critical(`Critical threshold exceeded for ${metric.name}`, undefined, 'PerformanceMonitor', {
        value: metric.value,
        threshold: threshold.critical,
        unit: metric.unit,
        tags: metric.tags,
      ...
      logger.warn(`Warning threshold exceeded for ${metric.name}`, 'PerformanceMonitor', {
        value: metric.value,
        threshold: threshold.warning,
        unit: metric.unit,
        tags: metric.tags,
  ...
  private generateRecommendations(summary: Record<string, any>, violations: any[]): string[] {
    const recommendations: string[] = [];
    
    // Response time recommendations
    if (summary.response_time && summary.response_time.avg > 2000) {
      recommendations.push('Consider optimizing database queries or adding caching to improve response times');
    }
    
    // Memory recommendations
    if (summary.memory_heap_used && summary.memory_heap_used.max > 200 * 1024 * 1024) {
    ...
    if (summary.error_rate && summary.error_rate.avg > 3) {
      recommendations.push('Elevated error rate detected. Review error logs and implement better error handling');
    }
    
    // Critical violations
    const criticalViolations = violations.filter(v => v.level === 'critical');
    if (criticalViolations.length > 0) {
      recommendations.push(`Address ${criticalViolations.length} critical performance issues immediately`);
    }
  ...
  private cleanup(): void {
    ...
    if (removed > 0) {
      logger.debug(`Cleaned up ${removed} old performance metrics`, 'PerformanceMonitor');
    }
  }
