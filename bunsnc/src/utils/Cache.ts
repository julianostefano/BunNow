...
export class Cache {
  ...
  cacheRecord(table: string, sysId: string, record: ServiceNowRecord, ttl?: number): void {
  ...
  cacheQuery(table: string, query: string, results: ServiceNowRecord[], ttl?: number): void {
  ...
  invalidateTable(table: string): void {
    ...
    for (const key of this.entries.keys()) {
      if (key.startsWith(`record:${table}:`) || key.startsWith(`query:${table}:`)) {
        keysToDelete.push(key);
      }
    }

    keysToDelete.forEach(key => this.delete(key));
    logger.info(`Invalidated ${keysToDelete.length} entries for table: ${table}`, 'Cache');
  }
  ...
  private shouldEvict(newEntry: CacheEntry): boolean {
    ...
    const currentMemory = Array.from(this.entries.values())
      .reduce((sum, entry) => sum + entry.size, 0);
  ...
  private cleanup(): void {
    ...
    if (keysToDelete.length > 0) {
      logger.debug(`Cache cleanup: removed ${keysToDelete.length} expired entries`, 'Cache');
    }
  }
