...
export class GlideRecord implements IGlideRecord {
  ...
  async update(): Promise<GlideElement | null> {
    ...
    try {
      ...
      const result = await this._client.serviceNow.update(
        this._table,
        sysId,
        data,
      );
          ...
          batch.addRequest({
            ...
            callback: (result, error) => {
              if (!error) {
                ...
                console.warn(
                  `Failed to delete record ${record.sys_id}:`,
                  error.message,
                );
              }
            },
          ...
          batch.addRequest({
            ...
            callback: (result, error) => {
              if (!error) {
                ...
                console.warn(
                  `Failed to update record ${record.sys_id}:`,
                  error.message,
                );
              }
            },
  ...
  addJoinQuery(
    ...
    return this._queryBuilder.addJoinQuery(
      joinTable,
      primaryField,
      joinTableField,
    );
  ...
  addRLQuery(
    ...
    return this._queryBuilder.addRLQuery(
      relatedTable,
      relatedField,
      operatorCondition,
      stopAtRelationship,
  ...
  private _fetchNextPageSync(): boolean {
    ...
    console.warn(
      "Synchronous pagination detected - consider using nextAsync() for better performance",
    );
    return false;
  }
    ...
    try {
      ...
      while (retryCount < maxRetries) {
        try {
          ...
          operation.progress(
            `Retrying page fetch (${retryCount}/${maxRetries}) in ${delay}ms`,
          );
      ...
      if (nextBatch && nextBatch.length > 0) {
        ...
        if (this._limit) {
          this._total = Math.min(
            this._limit,
            this._results.length + (this._hasMorePages ? this._batchSize : 0),
          );
        } else {
          this._total =
            this._results.length + (this._hasMorePages ? this._batchSize : 0);
  ...
  private _trimOldResults(): void {
    ...
    logger.debug(
      `Trimmed ${removedRecords.length} old records from memory`,
      "GlideRecord",
      {
        table: this._table,
