...
export class DataPartitioner extends EventEmitter {
  ...
  constructor(hdfs: HDFSClient, defaultConfig: PartitionConfig = {}) {
    ...
    logger.info('DataPartitioner initialized with strategy:', this.defaultConfig.strategy);
  }
  ...
  async createPartitionPlan(
    ...
    try {
      logger.info(`Creating partition plan for table ${table} with ${records.length} records`);
      ...
      switch (effectiveConfig.strategy) {
        case 'date':
          partitions.push(...await this.createDateBasedPartitions(table, records, effectiveConfig));
          break;
          
        case 'size':
          partitions.push(...await this.createSizeBasedPartitions(table, records, effectiveConfig));
          break;
          
        case 'record_count':
          partitions.push(...await this.createRecordCountBasedPartitions(table, records, effectiveConfig));
          break;
          
        case 'hybrid':
          partitions.push(...await this.createHybridPartitions(table, records, effectiveConfig));
          break;
      }
      ...
      logger.info(`Created partition plan: ${partitions.length} partitions, ${totalSize} bytes, ${totalRecords} records`);
  ...
  async executePartitionPlan(
    ...
    try {
        ...
        try {
          ...
          if (success) {
            ...
            const metadata: PartitionMetadata = {
              ...
              files: [],
              statistics: {
                avgRecordSize: partition.estimatedRecords > 0 ? 
                  partition.estimatedSize / partition.estimatedRecords : 0,
                hotness: 0
              }
            };
      ...
      if (success) {
        logger.info(`Successfully executed partition plan: ${successCount}/${plan.partitions.length} partitions created`);
      } else {
        logger.warn(`Partial success: ${successCount}/${plan.partitions.length} partitions created`);
      }
  ...
  async getOptimalPartition(
    ...
    switch (effectiveConfig.strategy) {
      ...
      case 'size':
        return await this.getSizeBasedPartitionPath(table, record, effectiveConfig);
        
      case 'record_count':
        return await this.getRecordCountBasedPartitionPath(table, record, effectiveConfig);
        
      case 'hybrid':
        return await this.getHybridPartitionPath(table, record, effectiveConfig);
  ...
  async compactPartitions(
    ...
    try {
      ...
      const smallPartitions = Array.from(this.partitionMetadata.values())
        .filter(p => p.path.includes(`/${table}/`) && p.sizeBytes < minPartitionSize)
      ...
      logger.info(`Found ${smallPartitions.length} small partitions to compact`);
      ...
      for (const group of compactionGroups) {
        try {
          const compactedPartition = await this.compactPartitionGroup(table, group);
  ...
  async cleanupExpiredPartitions(
    ...
    try {
      logger.info(`Cleaning up partitions older than ${retention} days for table ${table}`);
      
      const expiredPartitions = Array.from(this.partitionMetadata.values())
        .filter(p => 
      ...
      logger.info(`Found ${expiredPartitions.length} expired partitions to clean up`);
      ...
      for (const partition of expiredPartitions) {
        try {
          ...
          logger.error(`Failed to delete partition ${partition.partitionId}:`, error);
        }
      }
  ...
  async getPartitionStatistics(table: string): Promise<{
    ...
    const tablePartitions = Array.from(this.partitionMetadata.values())
    ...
    const smallPartitions = tablePartitions.filter(p => p.sizeBytes < smallThreshold).length;
    const largePartitions = tablePartitions.filter(p => p.sizeBytes > largeThreshold).length;
    const hotPartitions = tablePartitions.filter(p => (p.statistics?.hotness || 0) > 0.7).length;
    
    const timestamps = tablePartitions.map(p => p.createdAt);
    const oldestPartition = timestamps.length > 0 ? new Date(Math.min(...timestamps)) : null;
    const newestPartition = timestamps.length > 0 ? new Date(Math.max(...timestamps)) : null;
  ...
  updatePartitionAccess(partitionId: string): void {
    ...
    if (partition && partition.statistics) {
      partition.statistics.hotness = Math.min(1.0, partition.statistics.hotness + 0.1);
  ...
  private async createHybridPartitions(
    ...
    const datePartitions = await this.createDateBasedPartitions(table, records, config);
    ...
    for (const partition of datePartitions) {
      if (partition.estimatedSize > config.maxPartitionSize || 
          partition.estimatedRecords > config.maxRecordsPerPartition) {
        
        // Split large partition
        const subPartitions = await this.splitPartition(table, partition, config);
  ...
  private async splitPartition(
    ...
    const targetSubPartitions = Math.ceil(partition.estimatedSize / config.maxPartitionSize);
    const recordsPerSubPartition = Math.floor(partition.estimatedRecords / targetSubPartitions);
    const sizePerSubPartition = Math.floor(partition.estimatedSize / targetSubPartitions);
  ...
  private async getSizeBasedPartitionPath(
    ...
    const tablePartitions = this.getTablePartitions(table)
      .filter(p => p.sizeBytes < config.maxPartitionSize);
  ...
  private async getRecordCountBasedPartitionPath(
    ...
    const tablePartitions = this.getTablePartitions(table)
      .filter(p => p.recordCount < config.maxRecordsPerPartition);
  ...
  private async getHybridPartitionPath(
    ...
    const datePartitions = Array.from(this.partitionMetadata.values())
      .filter(p => p.path.startsWith(datePath));
    
    const availablePartition = datePartitions.find(p => 
  ...
  private extractRecordDate(record: any): Date {
    ...
    const dateFields = ['sys_updated_on', 'sys_created_on', 'opened_at', 'closed_at'];
  ...
  private estimateRecordsSize(records: any[]): number {
    return records.reduce((sum, record) => sum + this.estimateRecordSize(record), 0);
  }
}