...
export class HDFSClient extends EventEmitter {
  ...
  async uploadFile(
    ...
    try {
      ...
      const createUrl = this.buildUrl(hdfsPath, 'CREATE', {
        ...
        replication: (options.replication || this.config.replicationFactor).toString(),
      ...
      logger.info(`Successfully uploaded ${localPath} to HDFS ${hdfsPath} (${fileSize} bytes)`);
      ...
      this.emit('operation:error', { operation: 'upload', localPath, hdfsPath, error });
      return false;
    } finally {
  ...
  async downloadFile(
    ...
    try {
      ...
      logger.info(`Successfully downloaded ${hdfsPath} from HDFS to ${localPath} (${fileInfo.size} bytes)`);
      this.emit('file:downloaded', { hdfsPath, localPath, size: fileInfo.size });
      ...
      this.emit('operation:error', { operation: 'download', hdfsPath, localPath, error });
      return false;
    } finally {
      performanceMonitor.endTimer(timer);
    }
  ...
  async delete(
    ...
    try {
      ...
      if (success) {
        this.stats.successfulOperations++;
        logger.info(`Successfully deleted: ${hdfsPath} (recursive: ${recursive})`);
  ...
  async rename(oldPath: string, newPath: string): Promise<boolean> {
    ...
    try {
      ...
      this.emit('operation:error', { operation: 'rename', oldPath, newPath, error });
      return false;
    } finally {
      performanceMonitor.endTimer(timer);
    }
   ...
   */
  async setReplication(hdfsPath: string, replication: number): Promise<boolean> {
    ...
    try {
      ...
      if (success) {
        this.stats.successfulOperations++;
        logger.info(`Successfully set replication for ${hdfsPath} to ${replication}`);
  ...
  async getClusterSummary(): Promise<{
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Failed to get cluster summary: ${response.statusText}`);
      }
  ...
  private buildUrl(path: string, operation: string, params: Record<string, string> = {}): string {
...
export class ServiceNowHDFSUtils {
  ...
  async uploadParquetFile(
    ...
    await this.hdfs.createDirectory(parentDir, { ...options, permission: '755' });
  ...
  async listParquetFiles(
    ...
    while (currentDate <= endDate) {
      ...
      try {
        ...
        const parquetFiles = dayFiles.filter(file => 
          file.type === 'FILE' && file.path.endsWith('.parquet')
        );
        files.push(...parquetFiles);
      } catch (error) {
        // Directory might not exist for this date - skip
        logger.debug(`No data found for ${table} on ${currentDate.toISOString().split('T')[0]}`);
      }
  ...
  private async calculateDirectorySize(path: string): Promise<{ files: number; size: number }> {
    let totalSize = 0;
    let totalFiles = 0;
