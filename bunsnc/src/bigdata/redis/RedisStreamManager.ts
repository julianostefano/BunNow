...
export class RedisStreamManager extends EventEmitter {
  ...
  async readMessages(
    ...
    try {
      ...
      if (blockTime !== undefined) {
        // Blocking read
        result = await this.redis.xread('BLOCK', blockTime, 'STREAMS', streamKey, startId);
      } else {
        // Non-blocking read
  ...
  async startConsumer(options: ConsumerGroupOptions): Promise<RedisConsumer> {
    ...
    this.consumers.set(`${options.groupName}:${options.consumerName}`, consumer);
    ...
    logger.info(`Started consumer ${options.consumerName} in group ${options.groupName}`);
  ...
  async getStreamInfo(streamKey: string): Promise<StreamStats> {
    ...
    try {
      ...
      if (streamInfo.length > 0) {
        const firstEntry = await this.redis.xrange(streamKey, '-', '+', 'COUNT', 1);
        const lastEntry = await this.redis.xrevrange(streamKey, '+', '-', 'COUNT', 1);
        
        if (firstEntry.length > 0) {
          const [id, fields] = firstEntry[0];
          streamInfo.firstEntry = {
            id,
            timestamp: parseInt(this.parseFieldsArray(fields)._timestamp || '0'),
      ...
      if (streamInfo.firstEntry && streamInfo.lastEntry) {
        const timeDiff = (streamInfo.lastEntry.timestamp - streamInfo.firstEntry.timestamp) / 1000;
        streamInfo.messagesPerSecond = timeDiff > 0 ? streamInfo.totalMessages / timeDiff : 0;
      }
   ...
   */
  async trimStream(streamKey: string, maxLength: number, approximate: boolean = true): Promise<number> {
  ...
  async getPendingMessages(
    ...
    const pending = await this.redis.xpending(streamKey, groupName, '-', '+', 100);
   ...
   */
  async ackMessage(streamKey: string, groupName: string, messageId: string): Promise<number> {
    ...
    this.metricsTimer = setInterval(async () => {
      ...
      for (const streamKey of this.stats.keys()) {
        try {
          ...
          logger.debug(`Failed to update metrics for stream ${streamKey}:`, error);
        }
      }
...
export class RedisConsumer extends EventEmitter {
  ...
  private errorHandler?: (error: Error, message: StreamMessage) => Promise<boolean>;

  constructor(redis: RedisClient | RedisCluster, options: ConsumerGroupOptions) {
    super();
    ...
    logger.info(`RedisConsumer created: ${this.options.consumerName} in group ${this.options.groupName}`);
  }
   ...
   * Set error handler
   */
  onError(handler: (error: Error, message: StreamMessage) => Promise<boolean>): void {
    ...
    while (retryCount <= this.options.maxRetries) {
      try {
        ...
        if (this.options.autoAck) {
          await this.redis.xack(this.options.streamKey, this.options.groupName, message.id);
        }
      ...
      for (const item of pending) {
        ...
        if (idleTime > this.options.idleTime) {
          ...
          if (claimed && claimed.length > 0) {
            logger.info(`Claimed idle message ${messageId} from ${consumerName}`);
  ...
  private async sendToDeadLetter(message: StreamMessage, error: any): Promise<void> {
