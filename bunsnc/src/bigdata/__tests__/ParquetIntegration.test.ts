...
describe('ParquetWriter', () => {
  ...
  it('should support different compression types', async () => {
    ...
    for (const compressionType of compressionTypes) {
      ...
      const outputPath = path.join(testDataPath, `compression_${compressionType}.parquet`);
...
describe('ParquetReader', () => {
  ...
  it('should support filtering with predicates', async () => {
    const result = await reader.readRecordsWithFilter(testFilePath, {
      filters: [
        { column: 'priority', operator: 'equals', value: '1' }
      ]
    });
  ...
  it('should perform aggregations correctly', async () => {
    const result = await reader.aggregate(testFilePath, {
      ...
      aggregations: [
        ...
        { column: 'priority', function: 'distinct_count', alias: 'unique_priorities' }
      ]
    });
  ...
  beforeEach(async () => {
    ...
    mockGlideRecord = {
      ...
      getValue: jest.fn((field: string) => {
        ...
        return sampleIncidentData[recordIndex]?.[field as keyof typeof sampleIncidentData[0]];
      }),
...
describe('Parquet Performance Tests', () => {
  ...
  it('should handle very large datasets efficiently', async () => {
    ...
    const testFilePath = path.join(testDataPath, 'large_performance_test.parquet');
    
    // Write performance test
    const writeStartTime = Date.now();
    const writeStats = await writer.writeRecords(largeDataset, testFilePath, 'incident');
    ...
    expect(writeStats.performanceMetrics.avgRecordsPerSecond).toBeGreaterThan(1000);
  ...
  it('should demonstrate compression effectiveness', async () => {
    const compressibleData = Array.from({ length: 10000 }, (_, i) => ({
      ...
      description: 'This is a repeated description that should compress well due to its repetitive nature',
    ...
    const stats = await writer.writeRecords(compressibleData, testFilePath, 'incident');
