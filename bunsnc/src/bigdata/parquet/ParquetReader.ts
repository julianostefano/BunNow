...
export class ParquetReader {
  ...
  async readFile(filePath: string): Promise<any[]> {
    ...
    try {
      ...
      logger.info(`Read ${finalRecords.length} records from ${basename(filePath)}`);
  ...
  async* streamRecords(filePath: string): AsyncGenerator<any[], void, unknown> {
    ...
    try {
      ...
      logger.info(`Streaming ${totalRows} records from ${basename(filePath)} in batches of ${this.options.batchSize}`);
      
      // Process in batches
      for (let offset = 0; offset < totalRows; offset += this.options.batchSize) {
   ...
   */
  async query(filePath: string, queryOptions: ParquetQueryOptions): Promise<any[]> {
    ...
    try {
      ...
      let result = queryOptions.select 
        ? records.map(record => this.selectColumns(record, queryOptions.select!))
        : records;
      
      // Apply WHERE
      if (queryOptions.where) {
        result = result.filter(record => this.matchesWhere(record, queryOptions.where!));
      }
      
      // Apply GROUP BY and AGGREGATIONS
      if (queryOptions.groupBy || queryOptions.aggregations) {
        result = this.applyGroupByAndAggregations(result, queryOptions.groupBy, queryOptions.aggregations);
      }
   ...
   */
  async countRecords(filePath: string, whereClause?: Record<string, any>): Promise<number> {
    if (!whereClause) {
    ...
    for await (const batch of this.streamRecords(filePath)) {
      count += batch.filter(record => this.matchesWhere(record, whereClause)).length;
    }
   ...
   */
  async getDistinctValues(filePath: string, columnName: string, limit?: number): Promise<any[]> {
   ...
   */
  async getColumnStats(filePath: string, columnName: string): Promise<{
    count: number;
    sum: number;
    min: number;
    max: number;
  ...
  private applyGroupByAndAggregations(
    ...
    for (const record of records) {
      const groupKey = groupBy ? groupBy.map(field => record[field]).join('|') : 'all';
        ...
        for (const [field, aggType] of Object.entries(aggregations)) {
          ...
          switch (aggType) {
            case 'count':
              ...
              break;
            case 'sum':
              aggregatedRecord[`${field}_sum`] = values.reduce((a, b) => a + Number(b), 0);
              break;
            case 'avg':
              aggregatedRecord[`${field}_avg`] = values.length > 0 ? 
                values.reduce((a, b) => a + Number(b), 0) / values.length : 0;
              break;
            case 'min':
              aggregatedRecord[`${field}_min`] = values.length > 0 ? Math.min(...values.map(Number)) : null;
              break;
            case 'max':
              aggregatedRecord[`${field}_max`] = values.length > 0 ? Math.max(...values.map(Number)) : null;
              break;
          }
        }
      }
  ...
  private checkMemoryUsage(): void {
    ...
    if (memUsage.heapUsed > this.options.memoryLimit) {
      logger.warn(`Memory usage ${(memUsage.heapUsed / 1024 / 1024).toFixed(2)}MB exceeds limit ${(this.options.memoryLimit / 1024 / 1024).toFixed(2)}MB`);
    }
  }
  ...
  private calculateReadSpeed(): void {
    ...
    this.stats.averageReadSpeed = durationSeconds > 0 ? this.stats.recordsRead / durationSeconds : 0;
  }
