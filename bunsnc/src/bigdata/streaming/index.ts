...
export class ServiceNowStreamingPlatform {
  ...
  createProcessor(
    ...
    switch (type) {
      case "incident":
        processor =
          ServiceNowStreamProcessorFactory.createIncidentProcessor(config);
        break;
      case "export":
        processor =
          ServiceNowStreamProcessorFactory.createExportProcessor(config);
        break;
      case "notification":
        processor =
          ServiceNowStreamProcessorFactory.createNotificationProcessor(config);
        break;
      case "custom":
      default:
   ...
   */
  async createIncidentProcessingPipeline(
    config: {
      name?: string;
    ...
    const pipeline = this.createIncidentPipeline(processor, {
      openSearchIndexPrefix:
        config.openSearchIndexPrefix || "servicenow-incidents",
    ...
    if (this.redisStream) {
      await this.connectProcessorToRedisStream(
        processor,
        streamKey,
        processorName,
      );
  ...
  async createDataExportPipeline(config: {
    ...
    logger.info(
      `Data export pipeline created: ${processorName} for tables: ${config.tables.join(", ")}`,
    );
  ...
  getPlatformMetrics(): {
    ...
    const processorMetrics = Array.from(this.processors.entries()).map(
      ([name, processor]) => ({
        name,
    ...
    const avgErrorRate =
      processorMetrics.reduce((sum, p) => sum + p.metrics.errorRate, 0) /
        processorMetrics.length || 0;
    const avgBufferUtilization =
      processorMetrics.reduce(
        (sum, p) => sum + p.metrics.bufferUtilization,
        0,
      ) / processorMetrics.length || 0;
    ...
    if (unhealthyProcessors.length > 0) {
      overallHealth =
        unhealthyProcessors.length > processorMetrics.length / 2
          ? "critical"
          : "degraded";
      issues.push(
        `${unhealthyProcessors.length} unhealthy processors: ${unhealthyProcessors.map((p) => p.name).join(", ")}`,
      );
      recommendations.push(
        "Check processor logs and consider restarting unhealthy processors",
      );
    }
    ...
    if (avgBufferUtilization > 0.9) {
      ...
      issues.push(
        `High buffer utilization: ${(avgBufferUtilization * 100).toFixed(1)}%`,
      );
      recommendations.push(
        "Consider increasing buffer sizes or processing capacity",
      );
    }
  ...
  async startAll(): Promise<void> {
    ...
    logger.info(
      `Started ServiceNow Streaming Platform with ${this.processors.size} processors`,
    );
  }
  ...
  private createIncidentPipeline(processor: StreamProcessor, config: any): any {
    ...
    const enrichmentStream = processor.createTransformStream((record: any) => {
      ...
      return {
        ...
        search_text:
          `${record.short_description} ${record.description}`.toLowerCase(),
  ...
  private createExportPipeline(processor: StreamProcessor, config: any): any {
    ...
    const batchingStream = processor.createBatchingStream(
      config.batchSize || 1000,
    );

    const exportStream = processor.createProcessingStream(
      async (batches: any[][]) => {
        const results = [];
  ...
  private createNotificationProcessingPipeline(
    processor: StreamProcessor,
    config: any,
  ): any {
    // Rate limiting stream
    const rateLimitStream = processor.createRateLimitingStream(
      config.rateLimitPerMinute / 60,
    );

    // Notification processing
    const notificationStream = processor.createProcessingStream(
      async (batch: any[]) => {
        const results = [];
  ...
  private async connectProcessorToRedisStream(
    processor: StreamProcessor,
    streamKey: string,
  ...
  private updateGlobalMetrics(): void {
    ...
    this.globalMetrics = {
      ...
      avgThroughput:
        this.processors.size > 0 ? totalThroughput / this.processors.size : 0,
      lastUpdated: now,
    };
  }
