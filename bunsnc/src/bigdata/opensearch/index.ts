...
export { OpenSearchClient, ServiceNowOpenSearchIntegration } from './OpenSearchClient';
...
import { OpenSearchClient, ServiceNowOpenSearchIntegration } from './OpenSearchClient';
...
export class ServiceNowOpenSearchFactory {
   ...
   */
  async initializeServiceNowSearch(tables: string[] = ['incident', 'problem', 'change_request']): Promise<{
    success: boolean;
    ...
    try {
      ...
      if (!connected) {
        result.errors.push({ table: 'all', error: 'Cannot connect to OpenSearch cluster' });
        return result;
      }
      ...
      for (const table of tables) {
        try {
          ...
          const templateCreated = await this.indexManager.createServiceNowTemplate(table, {
            rolloverSize: '10gb',
            rolloverAge: '7d',
            replicas: 1,
            shards: 1
          ...
          if (templateCreated) {
            ...
            result.errors.push({ table, error: 'Failed to create index template' });
          }
  ...
  async indexServiceNowData(
    ...
    try {
      ...
      if (options.updateMappings && records.length > 0) {
        try {
          const mappingResult = await this.integration.updateDynamicMapping(table, records[0]);
          result.mappingsUpdated = mappingResult;
        } catch (error) {
          result.errors.push({
            operation: 'mapping_update',
            error: error instanceof Error ? error.message : 'Mapping update failed'
          });
        }
      }
  ...
  async generateAnalyticsDashboard(
    ...
    try {
      ...
      return {
        overview: { totalTickets: 0, openTickets: 0, resolvedTickets: 0, avgResolutionTime: 0 },
        trends: [],
        topCategories: []
      };
    }
   ...
   */
  async startComprehensiveMonitoring(options: {
    ...
    this.indexManager.on('index:unhealthy', (event) => {
      console.warn(`INDEX ALERT: ${event.indexName} is unhealthy (${event.status})`);
    });

    this.indexManager.on('index:performance:warning', (event) => {
      console.warn(`PERFORMANCE ALERT: ${event.indexName} - ${event.issue}: ${event.value}`);
    });
    ...
    if (options.alertThresholds) {
      const checkAndOptimize = async () => {
        try {
          const suggestions = await this.indexManager.getOptimizationSuggestions();
          const highPrioritySuggestions = suggestions.filter(s => s.priority === 'high');
          
          if (highPrioritySuggestions.length > 0) {
            console.log(`Auto-optimizing ${highPrioritySuggestions.length} high-priority issues`);
  ...
  async getHealthStatus(): Promise<{
    ...
    try {
      ...
      const servicenowMetrics = Array.from(indexMetrics.values())
        .filter(m => m.name.startsWith('servicenow-'));
      ...
      return {
        ...
        servicenow: {
          ...
          totalDocuments: servicenowMetrics.reduce((sum, m) => sum + m.documentsCount, 0),
          totalSize: servicenowMetrics.reduce((sum, m) => sum + m.totalSize, 0),
          healthyIndices: servicenowMetrics.filter(m => m.status === 'green').length
        },
        performance: {
          avgSearchLatency: servicenowMetrics.reduce((sum, m) => sum + m.searchRate, 0) / servicenowMetrics.length || 0,
          indexingRate: servicenowMetrics.reduce((sum, m) => sum + m.indexingRate, 0),
          searchRate: servicenowMetrics.reduce((sum, m) => sum + m.searchRate, 0)
        },
      ...
      return {
        ...
        servicenow: { totalIndices: 0, totalDocuments: 0, totalSize: 0, healthyIndices: 0 },
  ...
  private async generateSearchSuggestions(query: string, tables: string[]): Promise<string[]> {
    ...
    const commonTerms = [
      'incident', 'problem', 'change request', 'high priority', 'critical',
      'network issue', 'server down', 'application error', 'user access',
      'email problem', 'printer issue', 'software installation',
      'password reset', 'account locked', 'performance issue'
    ];
...
export const SERVICENOW_OPENSEARCH_DEFAULTS = {
  ...
  FIELD_MAPPINGS: {
    KEYWORD_FIELDS: ['sys_id', 'number', 'state', 'priority', 'assignment_group'],
    TEXT_FIELDS: ['short_description', 'description', 'work_notes'],
    DATE_FIELDS: ['sys_created_on', 'sys_updated_on', 'opened_at', 'resolved_at', 'closed_at']
  }
};