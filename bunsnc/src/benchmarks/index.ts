...
class BenchmarkRunner {
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      if (iterations > 100 && i % Math.floor(iterations / 10) === 0) {
        process.stdout.write(
          `\r  Progress: ${Math.floor((i / iterations) * 100)}%`,
        );
      }
    }
  ...
  getSummary(): BenchmarkSuite {
    ...
    const averageOps =
      this.results.reduce((sum, result) => sum + result.opsPerSecond, 0) /
      this.results.length;
  ...
  printSummary() {
    ...
    sorted.forEach((result, index) => {
      ...
      console.log(
        `${medal} ${result.name}: ${result.opsPerSecond.toFixed(2)} ops/sec (${relative.toFixed(1)}%)`,
      );
    });
...
async function runBenchmarks() {
  ...
  try {
    ...
    await runner.run(
      "Normal Query (50 records)",
      () => client.normalQuery(),
      300,
    );
    await runner.run(
      "Complex Query (100 records)",
      () => client.complexQuery(),
      200,
    );
    ...
    await runner.run(
      "Create Operation",
      () => client.create({ short_description: "Benchmark record" }),
      400,
    );

    await runner.run(
      "Update Operation",
      () => client.update("benchmark_id", { state: "2" }),
      400,
    );

    await runner.run(
      "Delete Operation",
      () => client.delete("benchmark_id"),
      400,
    );

    // Batch Operation Benchmarks
    await runner.run(
      "Small Batch (5 operations)",
      () => {
