...
export class ServiceNowEndpointMapper {
          ...
          if (value === null || value === undefined || value === "") {
            ...
            if (typeof value === "string") {
              ...
              fieldAnalysis.minLength = Math.min(
                fieldAnalysis.minLength || length,
                length,
              );
              fieldAnalysis.maxLength = Math.max(
                fieldAnalysis.maxLength || length,
                length,
              );
            }
          }
  ...
  async analyzeFieldTypes(records: any[]): Promise<FieldAnalysis[]> {
    ...
    allFields.forEach((fieldName) => {
      ...
      analysis.frequency =
        ((records.length - nullCount) / records.length) * 100;
  ...
  async testPerformanceLimits(table: string): Promise<{
    ...
    const recommendedLimit =
      successfulResults.find((r) => r.time > 5000)?.limit || maxLimit;
  ...
  async exportResults(): Promise<void> {
    ...
    const summary = {
      ...
      tablesWithErrors: this.testResults.filter((r) => r.status === "error")
        .length,
      avgResponseTime:
        this.testResults.reduce((sum, r) => sum + r.responseTime, 0) /
...
`;
  ...
  async analyzeSLATableRelationships(): Promise<{
    ...
    try {
          ...
          breachStats: {
            ...
            percentage:
              relationships.total > 0
                ? (breachedCount / relationships.total) * 100
                : 0,
          },
      ...
      console.log(
        `   Breach rate: ${result.slaPatterns.breachStats.percentage.toFixed(1)}%`,
      );
      console.log(
        `   Common SLAs: ${commonSLAs
          .slice(0, 3)
          .map((s) => s.name)
          .join(", ")}`,
      );
