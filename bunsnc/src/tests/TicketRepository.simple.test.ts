...
describe("TicketRepository Simple Tests", () => {
  ...
  it("should reject invalid incident without caller_id", async () => {
    ...
    await expect(repository.saveTicket(invalidIncident)).rejects.toThrow(
      "Missing required field: caller_id",
    );
  });
  ...
  it("should reject invalid state for incident", async () => {
    ...
    await expect(repository.saveTicket(invalidStateIncident)).rejects.toThrow(
      "Invalid state '99' for table incident",
    );
  });
  ...
  it("should reject invalid priority", async () => {
    ...
    await expect(
      repository.saveTicket(invalidPriorityIncident),
  ...
  it("should update ticket with audit logging", async () => {
    ...
    await expect(
      repository.updateTicket("test-123", "incident", updates, "admin"),
  ...
  it("should throw error updating non-existent ticket", async () => {
    ...
    await expect(
      repository.updateTicket("non-existent", "incident", updates),
    ).rejects.toThrow("Ticket incident/non-existent not found");
  });

  it("should mark sync status correctly", async () => {
    await expect(
      repository.markSyncStatus("test-123", "incident", "error", "Sync failed"),
    ).resolves.toBeUndefined();

    await expect(
      repository.markSyncStatus("test-123", "incident", "synced"),
    ).resolves.toBeUndefined();
  });
  ...
  it("should throw error for unsupported table type", async () => {
    ...
    await expect(repository.saveTicket(invalidTable)).rejects.toThrow(
      "Unsupported table type: unsupported_table",
    );
  });
