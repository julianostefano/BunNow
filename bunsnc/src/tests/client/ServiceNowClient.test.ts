...
describe('ServiceNowClient', () => {
  ...
  describe('Construction', () => {
    ...
    test('should normalize instance URL by removing trailing slash', () => {
      const clientWithSlash = new ServiceNowClient('https://dev12345.service-now.com/', authToken);
  ...
  describe('Static Factory Methods', () => {
    ...
    test('should throw error when environment variables are missing', () => {
      ...
      expect(() => ServiceNowClient.fromEnv()).toThrow('SERVICENOW_INSTANCE environment variable is required');
      
      process.env.SERVICENOW_INSTANCE = instanceUrl;
      expect(() => ServiceNowClient.fromEnv()).toThrow('SERVICENOW_TOKEN environment variable is required');
  ...
  describe('CRUD Operations', () => {
    ...
    test('should create record', async () => {
      const newRecord = { sys_id: 'new123', short_description: 'Test incident' };
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: () => Promise.resolve({ result: newRecord })
      } as any);

      const result = await client.create('incident', { short_description: 'Test incident' });
  ...
  describe('Attachment Operations', () => {
    test('should upload attachment', async () => {
      mockFetch.mockResolvedValueOnce({
        ok: true,
        headers: new Map([['Location', '/api/now/attachment/attachment123/file']])
      } as any);

      const file = new Blob(['test content'], { type: 'text/plain' });
      const result = await client.uploadAttachment('test.txt', 'incident', 'inc123', file);
  ...
  describe('Batch Operations', () => {
    test('should execute sequence of operations', async () => {
      ...
      mockFetch
        ...
        .mockResolvedValueOnce({
          ok: true,
          json: () => Promise.resolve({ result: { sys_id: 'inc2', short_description: 'Test' } })
        } as any);
