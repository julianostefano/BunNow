...
export class AIServicesTestSuite {
  ...
  async runAllTests(): Promise<TestResult[]> {
    logger.info(
      " [AIServicesTest] Starting comprehensive AI services test suite...",
    );
  ...
  private async testTikaClient(): Promise<void> {
    ...
    const testBuffer = Buffer.from(
      "This is a test document for Tika processing.",
    );
  ...
  private async testLLMClient(): Promise<void> {
    ...
    await this.runTest("LLMClient", "Simple Completion", async () => {
      try {
        const response = await client.generateCompletion("What is 2+2?", {
          temperature: 0.1,
          max_tokens: 50,
        });
    ...
    await this.runTest("LLMClient", "Ticket Analysis", async () => {
      try {
        ...
        const analysis = await client.analyzeTicket(ticketData, {
          temperature: 0.3,
        });
    ...
    await this.runTest("LLMClient", "Document Summarization", async () => {
      try {
        const longText =
          "This is a long technical document that needs to be summarized. " +
    ...
    await this.runTest("LLMClient", "Keyword Extraction", async () => {
      try {
        const text =
          "Database connection error occurred while connecting to Oracle server. " +
  ...
  private printSummary(): void {
    ...
    // Group by service
    const serviceResults = this.results.reduce(
      (acc, result) => {
    ...
    for (const [service, stats] of Object.entries(serviceResults)) {
      ...
      logger.info(
        `  ${service}: ${stats.passed}/${stats.total} (${successRate}%)`,
      );
    }
    ...
    if (failedTests.length > 0) {
      ...
      failedTests.forEach((test) => {
        logger.warn(
          `  ${test.service} - ${test.test}: ${test.error || "Unknown error"}`,
        );
      });
    }
  }
