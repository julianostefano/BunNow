...
describe('ServiceNowClient Performance Tests', () => {
  ...
  describe('Query Performance', () => {
    ...
    test('should demonstrate query performance scaling', async () => {
      ...
      results.forEach(result => {
        console.log(`  ${result.size} records: ${result.duration.toFixed(2)}ms (${result.rps.toFixed(2)} RPS)`);
      });
    });
  });
  ...
  describe('Concurrent Operation Performance', () => {
    test('should handle concurrent queries efficiently', async () => {
      ...
      results.forEach(result => {
        console.log(`  ${result.concurrency} concurrent: ${result.totalTime.toFixed(2)}ms total, ${result.avgTime.toFixed(2)}ms avg`);
      });
    });
  ...
  describe('Batch Operation Performance', () => {
    ...
    test('should handle large batch operations efficiently', async () => {
      ...
      results.forEach(result => {
        console.log(`  ${result.size} records: ${result.duration.toFixed(2)}ms (${result.throughput.toFixed(2)} RPS)`);
      });
  ...
  describe('Error Handling Performance', () => {
    test('should handle errors without significant performance impact', async () => {
      ...
      console.log(`Error Handling: ${successes.length} successes, ${errors.length} errors in ${duration.toFixed(2)}ms`);
    ...
    test('should demonstrate retry performance impact', async () => {
      ...
      console.log(`Retry Performance: ${results.filter(r => !('error' in r)).length}/5 succeeded in ${duration.toFixed(2)}ms`);
  ...
  describe('Memory Performance', () => {
    test('should handle large result sets without memory leaks', async () => {
      ...
      for (let i = 0; i < 5; i++) {
        const queryResults = await client.query({ table: 'incident', limit: 500 });
    ...
    test('should demonstrate garbage collection efficiency', async () => {
      ...
      for (let i = 0; i < 10; i++) {
        ...
        const largeResult = await client.query({ table: 'incident', limit: 200 });
      ...
      console.log(`Memory Stability: ${(memoryGrowth / 1024 / 1024).toFixed(2)}MB growth over 10 iterations`);
    });
  });
  ...
  describe('Performance Regression Tests', () => {
    test('should maintain baseline performance standards', async () => {
      ...
      performanceMonitor.startTimer(batchTimer, 'PerformanceTest');
      await client.batchQuery(Array.from({ length: 10 }, (_, i) => ({
        table: 'incident',
        limit: 10,
        offset: i * 10
      })));
  ...
  describe('Stress Testing', () => {
    test('should handle sustained load', async () => {
      ...
      const p95ResponseTime = operations.sort((a, b) => a - b)[Math.floor(operations.length * 0.95)];
