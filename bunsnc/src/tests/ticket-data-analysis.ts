...
import {
  ServiceNowEndpointMapper,
  TableSchema,
  FieldAnalysis,
} from "./ServiceNowEndpointMapper";
...
class TicketDataAnalyzer {
  ...
  async analyzeTicketTable(
    ...
    try {
      ...
      const testResult = await this.mapper.testEndpoint(
        tableName,
        undefined,
        100,
      );
      ...
      const analysis: TicketAnalysis = {
        ...
        statusValues: this.extractUniqueValues(records, [
          "status",
          "state",
          "incident_state",
        ]),
        priorityValues: this.extractUniqueValues(records, [
          "priority",
          "urgency",
          "impact",
        ]),
        stateValues: this.extractUniqueValues(records, [
          "state",
          "ticket_state",
          "work_state",
        ]),
        assignmentGroups: this.extractUniqueValues(records, [
          "assignment_group",
  ...
  async compareTicketTables(): Promise<TicketComparison> {
    ...
    this.ticketAnalyses.forEach((analysis) => {
      const uniqueFields = Array.from(allFieldsMap.entries())
        .filter(
          ([field, tables]) =>
            tables.length === 1 && tables[0] === analysis.tableName,
        )
        .map(([field]) => field);
    ...
    const comparison: TicketComparison = {
      commonToAll,
      commonToSome: commonToSome.sort(
    ...
    console.log(
      `   Business Logic Fields: ${businessLogicFields.length} fields`,
    );
  ...
  async exportAnalysisResults(results: {
    ...
    const recommendationsFile = join(
      this.outputDir,
      "storage-recommendations.json",
    );
    writeFileSync(
      recommendationsFile,
      JSON.stringify(
  ...
  private identifyCommonTicketFields(fields: FieldAnalysis[]): string[] {
    ...
    return fields
      .filter((f) =>
        commonPatterns.some((pattern) => f.fieldName.includes(pattern)),
      )
      .map((f) => f.fieldName);
  }

  private identifyUniqueFields(
  ...
  private identifyDateFields(fields: FieldAnalysis[]): string[] {
    return fields
      .filter(
        (f) =>
          f.dataType === "date" ||
          f.fieldName.includes("_at") ||
          f.fieldName.includes("_date"),
      )
      .map((f) => f.fieldName);
  }
    ...
    return fields
      .filter(
        (f) =>
          ...
          f.fieldName.includes("_id") ||
          f.fieldName.includes("sys_id"),
      )
      .map((f) => f.fieldName);
  }

  private identifyBusinessFields(
    fields: FieldAnalysis[],
    ticketType: string,
  ): string[] {
  ...
  private identifyBusinessLogicFields(): {
    field: string;
    purpose: string;
    tables: string[];
  }[] {
    const businessLogic = [
      ...
      },
      {
        ...
        purpose: "Team assignment and routing",
        tables: ["incident", "change_task", "sc_task"],
      },
      {
        field: "priority",
      ...
      },
      {
        ...
        tables: ["change_task", "sc_task"],
      },
      {
        field: "business_service",
        purpose: "Service impact assessment",
        tables: ["incident", "change_task"],
      },
    ];
  ...
  private generateStorageRecommendations(
    ...
    return {
      sharedSchema: commonToAll.filter(
        (field) =>
          !field.includes("sys_") ||
          ["sys_id", "sys_created_on", "sys_updated_on"].includes(field),
      ),
  ...
  private async generateMongoDBSchemas(results: {
    ...
    const baseSchema = {
      bsonType: "object",
      required: results.comparison.storageRecommendations.sharedSchema.slice(
        0,
        10,
      ),
      properties: results.comparison.storageRecommendations.sharedSchema.reduce(
        (props: any, field) => {
    ...
    results.analyses.forEach((analysis) => {
      const specificFields =
        results.comparison.storageRecommendations.tableSpecific.find(
          (ts) => ts.table === analysis.tableName,
        )?.fields || [];
...
async function main() {
  ...
  if (!instanceUrl || !token) {
    console.error(
      " Please set SERVICENOW_INSTANCE_URL and authentication credentials",
    );
    console.error(
  ...
  try {
    ...
    results.analyses.forEach((analysis) => {
      console.log(
        `   ${analysis.tableName}: ${analysis.totalRecords} records, ${analysis.commonFields.length + analysis.uniqueFields.length} fields`,
      );
    });
    ...
    if (results.comparison) {
      console.log(`\n Field Distribution:`);
      console.log(
        `   Common to all tables: ${results.comparison.commonToAll.length}`,
      );
      console.log(
        `   Partially shared: ${results.comparison.commonToSome.length}`,
      );
      console.log(
        `   Table-specific: ${results.comparison.uniquePerTable.reduce((sum, t) => sum + t.fields.length, 0)}`,
      );
    }
  } catch (error: any) {
