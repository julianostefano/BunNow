...
import { QueryBuilder, QueryCondition, OrCondition, JoinQuery, RLQuery } from '../../query';
...
describe('QueryBuilder', () => {
  ...
  describe('Related List Queries', () => {
    test('should create basic RL query', () => {
      const rlQuery = queryBuilder.addRLQuery('task_ci', 'ci_item', 'some_condition');
      const query = queryBuilder.generateQuery();
      expect(query).toBe('RLQUERYtask_ci.ci_item,some_condition^ENDRLQUERY');
    });

    test('should create RL query with stop at relationship', () => {
      const rlQuery = queryBuilder.addRLQuery('task_ci', 'ci_item', 'some_condition', true);
      const query = queryBuilder.generateQuery();
      expect(query).toBe('RLQUERYtask_ci.ci_item,some_condition,m2m^ENDRLQUERY');
    });
    ...
    test('should create RL query with conditions', () => {
      ...
      const query = queryBuilder.generateQuery();
      expect(query).toBe('RLQUERYtask_ci.ci_item,some_condition^active=true^ENDRLQUERY');
    });
  });
  ...
  describe('Complex Queries', () => {
    test('should combine multiple query types', () => {
      ...
      const query = queryBuilder.generateQuery();
      expect(query).toBe('state=1^active=true^priority=1^ORpriority=2^ORDERBYnumber');
    });
  ...
  describe('Error Handling', () => {
    ...
    test('should throw error for RL query without table', () => {
      const qbWithoutTable = new QueryBuilder();
      expect(() => qbWithoutTable.addRLQuery('task_ci', 'ci_item', 'condition')).toThrow(
