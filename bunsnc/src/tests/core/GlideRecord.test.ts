...
class MockServiceNowClient {
  ...
  async update(table: string, sysId: string, data: ServiceNowRecord): Promise<ServiceNowRecord> {
    ...
    if (index >= 0) {
      records[index] = { ...records[index], ...data, sys_updated_on: new Date().toISOString() };
  ...
  private applyQuery(records: ServiceNowRecord[], query: string): ServiceNowRecord[] {
  ...
  private applyOrdering(records: ServiceNowRecord[], orderBy: string): ServiceNowRecord[] {
...
describe('GlideRecord Advanced Functionality', () => {
  ...
  describe('Basic Query Operations', () => {
    ...
    test('should query records with complex query operators', async () => {
      ...
      expect(records.every(r => ['1', '2', '3'].includes(r.priority))).toBe(true);
    });
    ...
    test('should query records with CONTAINS operator', async () => {
      ...
      expect(records.length).toBeGreaterThan(0);
      expect(records.every(r => r.short_description.includes('Test'))).toBe(true);
    });
  ...
  describe('Advanced Query Features', () => {
    test('should support dynamic query building', async () => {
      ...
      conditions.forEach(condition => {
        glideRecord.addQuery(condition.field, condition.operator, condition.value);
      });
    ...
    test('should support saved queries', async () => {
      ...
      expect(records.length).toBeGreaterThan(0);
      expect(records.every(r => r.state !== '6' && r.state !== '7')).toBe(true);
      
      newGlideRecord.destroy();
    });

    test('should support query caching', async () => {
      const cachingGlideRecord = new GlideRecord(mockClient as any, 'incident', {
        enableCaching: true,
        cacheTimeout: 5000
      });
  ...
  describe('Ordering and Pagination', () => {
    ...
    test('should support auto-pagination', async () => {
      ...
      expect(allRecords.map(r => r.sys_id)).toEqual(['1', '2', '3', '4', '5']);
  ...
  describe('Field Operations', () => {
    ...
    test('should set field values', () => {
      ...
      expect(glideRecord.getValue('short_description')).toBe('Updated description');
  ...
  describe('Cross-Table Operations', () => {
    ...
    test('should support joins across tables', async () => {
      ...
      const joinedResults = await joinGR.join('problem', 'assigned_to', 'assigned_to');
    ...
    test('should support related record queries', async () => {
      ...
      const relatedProblems = await glideRecord.getRelatedRecords('problem', 'assigned_to');
