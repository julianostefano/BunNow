...
const mockClient = {
  ...
  serviceNow: {
    ...
    read: mock(() =>
      Promise.resolve({ sys_id: "test_id", name: "Test Record" }),
    ),
    update: mock(() =>
      Promise.resolve({ sys_id: "test_id", name: "Updated Record" }),
    ),
...
describe("GlideRecord", () => {
  ...
  describe("Basic Construction", () => {
    ...
    test("should create with rewindable setting", () => {
      ...
      expect(() => nonRewindable.rewind()).toThrow(
        "This GlideRecord is not rewindable",
      );
    });
  });
  ...
  describe("Properties", () => {
    ...
    test("should get and set display value", () => {
      ...
      expect(() => {
        gr.displayValue = "invalid" as any;
      }).toThrow();
    });
  ...
  describe("Query Building", () => {
    ...
    test("should add JOIN queries", () => {
      ...
      const query = gr.getEncodedQuery();
      expect(query).toBe(
        "JOINincident.assigned_to=sys_user.sys_id!active=true",
      );
    });
    ...
    test("should add RL queries", () => {
      ...
      const query = gr.getEncodedQuery();
      expect(query).toBe(
        "RLQUERYtask_ci.ci_item,category=hardware^active=true^ENDRLQUERY",
      );
    });
  ...
  describe("CRUD Operations", () => {
    ...
    test("should update existing record", async () => {
      ...
      expect(result).toBeInstanceOf(GlideElement);
      expect(mockClient.serviceNow.update).toHaveBeenCalledWith(
        "incident",
        "test_id",
        {
    ...
    test("should delete record", async () => {
      ...
      expect(result).toBe(true);
      expect(mockClient.serviceNow.delete).toHaveBeenCalledWith(
        "incident",
        "test_id",
      );
    });
    ...
    test("should throw error when updating without current record", async () => {
      await expect(gr.update()).rejects.toThrow(
        "Cannot update: no current record",
      );
    });

    test("should throw error when deleting without current record", async () => {
      await expect(gr.delete()).rejects.toThrow(
        "Cannot delete: no current record",
      );
    });
  });
  ...
  describe("Error Handling", () => {
    ...
    test("should handle invalid location setting", () => {
      expect(() => {
        gr.location = 5;
      }).toThrow("No location to be had when we have no query");

      gr["_total"] = 3;
      expect(() => {
        gr.location = -2;
      }).toThrow("Location out of bounds");
      expect(() => {
        gr.location = 3;
      }).toThrow("Location out of bounds");
    });
