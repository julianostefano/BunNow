...
describe("TableAPI", () => {
  ...
  describe("Update Record", () => {
    ...
    test("should handle update errors", async () => {
      ...
      await expect(
        tableAPI.update("incident", "test123", {}),
      ).rejects.toThrow();
    });
  });
  ...
  describe("Batch Operations", () => {
    test("should execute batch operations sequentially", async () => {
      ...
      mockFetch
        ...
        .mockResolvedValueOnce({
          ...
          json: () =>
            Promise.resolve({
              result: { sys_id: "inc4", short_description: "Test" },
            }),
        } as any)
        .mockResolvedValueOnce({
          ok: true,
          json: () =>
            Promise.resolve({ result: { sys_id: "inc2", state: "2" } }),
        } as any)
        .mockResolvedValueOnce({
          ok: true,
          status: 204,
    ...
    test("should handle list operations in batch", async () => {
      ...
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: () =>
          Promise.resolve({ result: [{ sys_id: "inc1" }, { sys_id: "inc2" }] }),
      } as any);
  ...
  describe("Statistics and Health", () => {
    test("should get instance statistics", async () => {
      mockFetch.mockResolvedValueOnce({
        ...
        json: () =>
          Promise.resolve({
            result: [{ name: "glide.war", value: "San Diego" }],
          }),
      } as any);
