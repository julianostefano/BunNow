...
describe('ErrorHandler', () => {
  ...
  describe('Basic Functionality', () => {
    ...
    test('should determine retryability correctly', () => {
      ...
      const error500 = handler.createError(500, 'Internal Server Error', context);
  ...
  describe('Retry Logic', () => {
    test('should retry retryable errors with exponential backoff', async () => {
      ...
      const failingOperation = mock(() => {
        attempts++;
        if (attempts < 3) {
          throw new ServiceNowError('Temporary failure', 503, context, undefined, undefined, true);
        }
      ...
      const result = await handler.handleError(
        new ServiceNowError('Initial failure', 503, context, undefined, undefined, true),
        context,
        failingOperation
      );
    ...
    test('should not retry non-retryable errors', async () => {
      ...
      const failingOperation = mock(() => {
        attempts++;
        return Promise.reject(new ServiceNowError('Not found', 404, context, undefined, undefined, false));
      });
      
      await expect(handler.handleError(
        new ServiceNowError('Not found', 404, context, undefined, undefined, false),
        context,
        failingOperation
    ...
    test('should stop retrying if error becomes non-retryable', async () => {
      ...
      const failingOperation = mock(() => {
        ...
        if (attempts === 1) {
          ...
          throw new ServiceNowError('Server error', 503, context, undefined, undefined, true);
        } else {
          // Second attempt: non-retryable error
          throw new ServiceNowError('Unauthorized', 401, context, undefined, undefined, false);
        }
      });
      
      await expect(handler.handleError(
        new ServiceNowError('Server error', 503, context, undefined, undefined, true),
        context,
        failingOperation
    ...
    test('should respect maximum retry limit', async () => {
      ...
      const alwaysFailingOperation = mock(() => {
        attempts++;
        throw new ServiceNowError('Always fails', 503, context, undefined, undefined, true);
      });
      
      await expect(handler.handleError(
        new ServiceNowError('Always fails', 503, context, undefined, undefined, true),
        context,
  ...
  describe('Recovery Strategies', () => {
    ...
    test('should fall back to retry if no recovery strategies work', async () => {
      ...
      const retryOperation = mock(() => {
        attempts++;
        if (attempts < 2) {
          throw new ServiceNowError('Retry attempt', 503, context, undefined, undefined, true);
        }
        return Promise.resolve('success after retry');
      });
      
      const result = await handler.handleError(
        new ServiceNowError('Initial error', 503, context, undefined, undefined, true),
        context,
        retryOperation
      );
  ...
  describe('Statistics and Monitoring', () => {
    test('should provide detailed error statistics', async () => {
      ...
      try {
        await handler.handleError(
          new ServiceNowError('Server error', 500, context, undefined, undefined, true),
    ...
    test('should calculate recovery and retry rates correctly', async () => {
      ...
      let attempts = 0;
      await handler.handleError(
        new ServiceNowError('Retryable error', 503, context, undefined, undefined, true),
        context,
        mock(() => {
          attempts++;
          if (attempts < 2) {
  ...
  describe('Error Handling Edge Cases', () => {
    test('should handle null/undefined errors gracefully', async () => {
      ...
      // Test null error
      await expect(handler.handleError(null as any, context, mockOperation))
        .rejects.toThrow();
      
      // Test undefined error
      await expect(handler.handleError(undefined as any, context, mockOperation))
        .rejects.toThrow();
    });
    ...
    test('should handle string errors', async () => {
      ...
      await expect(handler.handleError('String error' as any, context, mockOperation))
    ...
    test('should handle errors without status codes', async () => {
      ...
      await expect(handler.handleError(genericError, context, mockOperation))
    ...
    test('should handle recovery strategy exceptions gracefully', async () => {
      ...
      let attempts = 0;
      const result = await handler.handleError(
        new ServiceNowError('Test error', 503, context, undefined, undefined, true),
        context,
        mock(() => {
          attempts++;
          if (attempts < 2) {
  ...
  describe('Default Recovery Strategies', () => {
    ...
    test('should handle rate limit errors with backoff', async () => {
      ...
      await expect(handler.handleError(
        new ServiceNowError('Rate limited', 429, context, undefined, { retryAfter: 1 }, true),
        context,
        mockOperation
