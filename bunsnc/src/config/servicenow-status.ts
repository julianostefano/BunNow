...
export const TICKET_TYPES: Record<string, TicketTypeConfig> = {
  incident: {
    ...
    fields: [
      'sys_id', 'number', 'state', 'short_description', 'description',
      'priority', 'urgency', 'impact', 'category', 'subcategory',
      'assignment_group', 'assigned_to', 'caller_id', 'opened_by',
      'sys_created_on', 'sys_updated_on', 'due_date', 'business_service',
      'cmdb_ci', 'close_notes', 'work_notes'
    ],
    detailFields: [
      'number', 'state', 'short_description', 'description', 'priority',
      'urgency', 'impact', 'category', 'subcategory', 'assignment_group', 
      'assigned_to', 'caller_id', 'opened_by', 'sys_created_on',
      'sys_updated_on', 'due_date', 'business_service', 'cmdb_ci',
      'close_notes', 'work_notes', 'correlation_id', 'correlation_display'
    ],
    actions: {
      assign: { label: 'Assumir', icon: 'user-check', targetStatus: '2', requiresNote: false },
      note: { label: 'Anotar', icon: 'message-square', requiresNote: true },
      in_progress: { label: 'Colocar em Andamento', icon: 'play', targetStatus: '2', requiresNote: false },
      hold: { label: 'Colocar em Espera', icon: 'pause', targetStatus: '3', requiresNote: true },
      resolve: { label: 'Resolver', icon: 'check-circle', targetStatus: '6', requiresNote: true },
      close: { label: 'Fechar', icon: 'x-circle', targetStatus: '7', requiresNote: false },
      cancel: { label: 'Cancelar', icon: 'ban', targetStatus: '8', requiresNote: true, confirmMessage: 'Deseja realmente cancelar este incidente?' },
      reopen: { label: 'Reabrir', icon: 'refresh-cw', targetStatus: '2', requiresNote: true }
    }
  },
  change_task: {
    name: 'change_task',
    ...
    fields: [
      'sys_id', 'number', 'state', 'short_description', 'description',
      'assignment_group', 'assigned_to', 'change_request', 'type',
      'sys_created_on', 'sys_updated_on', 'due_date', 'planned_start_date',
      'planned_end_date', 'work_notes', 'close_notes'
    ],
    detailFields: [
      'number', 'state', 'short_description', 'description', 'assignment_group',
      'assigned_to', 'change_request', 'type', 'sys_created_on', 'sys_updated_on',
      'due_date', 'planned_start_date', 'planned_end_date', 'work_notes', 'close_notes'
    ],
    actions: {
      assign: { label: 'Assumir', icon: 'user-check', targetStatus: '2', requiresNote: false },
      note: { label: 'Anotar', icon: 'message-square', requiresNote: true },
      in_progress: { label: 'Colocar em Andamento', icon: 'play', targetStatus: '3', requiresNote: false },
      complete: { label: 'Completar', icon: 'check-circle', targetStatus: '4', requiresNote: false },
      skip: { label: 'Pular', icon: 'skip-forward', targetStatus: '7', requiresNote: true, confirmMessage: 'Deseja pular esta tarefa?' },
      incomplete: { label: 'Fechar Incompleto', icon: 'x-circle', targetStatus: '8', requiresNote: true, confirmMessage: 'Deseja fechar esta tarefa como incompleta?' }
    }
  },
  sc_task: {
    name: 'sc_task',
    ...
    fields: [
      'sys_id', 'number', 'state', 'short_description', 'description',
      'assignment_group', 'assigned_to', 'request', 'request_item',
      'sys_created_on', 'sys_updated_on', 'due_date', 'work_notes', 'close_notes'
    ],
    detailFields: [
      'number', 'state', 'short_description', 'description', 'assignment_group',
      'assigned_to', 'request', 'request_item', 'sys_created_on', 'sys_updated_on',
      'due_date', 'work_notes', 'close_notes'
    ],
    actions: {
      assign: { label: 'Assumir', icon: 'user-check', targetStatus: '2', requiresNote: false },
      note: { label: 'Anotar', icon: 'message-square', requiresNote: true },
...
// Helper functions
export function getStatusConfig(ticketType: string, stateValue: string): StatusConfig | null {
...
export function canTransitionTo(ticketType: string, fromState: string, toState: string): boolean {
...
export function getUserActions(ticketType: string, currentState: string): string[] {
