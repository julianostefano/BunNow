...
export class TaskManager extends EventEmitter {
      ...
      case 'task.updated':
        if (event.data.task.status === TaskStatus.COMPLETED) {
          ...
          if (event.data.task.startedAt && event.data.task.completedAt) {
            const duration = event.data.task.completedAt.getTime() - event.data.task.startedAt.getTime();
   ...
   */
  async setScheduledTaskEnabled(taskId: string, enabled: boolean): Promise<void> {
  ...
  async exportToParquet(
    ...
    return await this.addTask(
      TaskType.PARQUET_EXPORT,
      {
        ...
        outputPath: this.config.integrations.parquet?.outputPath || '/tmp/parquet',
  ...
  async getSystemStats(): Promise<{
    ...
    const successRate = this.metrics.tasksCreated > 0 
  ...
  async getTaskHistory(limit: number = 100): Promise<Task[]> {
    const completed = await this.getTasks(TaskStatus.COMPLETED, Math.floor(limit / 2));
    const failed = await this.getTasks(TaskStatus.FAILED, Math.floor(limit / 2));
