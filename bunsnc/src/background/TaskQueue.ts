...
export class TaskQueue extends EventEmitter {
   ...
   */
  async addTask(task: Omit<Task, 'id' | 'status' | 'progress' | 'retryCount' | 'createdAt'>): Promise<string> {
  ...
  async getTask(taskId: string): Promise<Task | null> {
    try {
      const taskData = await this.redis.hGet(`${this.TASK_DATA_PREFIX}${taskId}`, 'data');
  ...
  async updateTask(taskId: string, updates: Partial<Task>): Promise<void> {
    try {
      ...
      await this.moveTaskBetweenQueues(taskId, currentTask.status, updatedTask.status);
  ...
  async getTasks(
    ...
    try {
      ...
      const taskIds = await this.redis.zRange(queueName, offset, offset + limit - 1);
  ...
  async getStats(): Promise<{
    ...
    try {
      ...
      const recentCompleted = await this.redis.zRange(this.COMPLETED_QUEUE, -100, -1);
  ...
  private async moveTaskBetweenQueues(
    ...
    const getQueueName = (status: TaskStatus): string => {
      switch (status) {
        case TaskStatus.PENDING: return this.PENDING_QUEUE;
        case TaskStatus.RUNNING: return this.RUNNING_QUEUE;
        case TaskStatus.COMPLETED: return this.COMPLETED_QUEUE;
        case TaskStatus.FAILED: return this.FAILED_QUEUE;
        default: return this.PENDING_QUEUE;
      }
    };
  ...
  async getNextTask(): Promise<Task | null> {
    try {
      ...
      if (task) {
        ...
        await this.redis.zAdd(this.RUNNING_QUEUE, { score: Date.now(), value: taskId });
      }
...
export class TaskWorker {
  ...
  async cancel(): Promise<void> {
    if (this.currentTask) {
      console.log(`⏹️ Cancelling task ${this.currentTask.id} on worker ${this.workerId}`);
