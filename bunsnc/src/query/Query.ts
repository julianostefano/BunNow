...
export class Query {
   ...
   * Add a JOIN query
   */
  addJoinQuery(joinTable: string, primaryField?: string, joinTableField?: string): any {
    if (!this._table) {
      throw new Error('Cannot execute join query as Query object was not instantiated with a table name');
    }
    
    // Import dynamically to avoid circular dependency
    const { JoinQuery } = require('./JoinQuery');
    const joinQuery = new JoinQuery(this._table, joinTable, primaryField, joinTableField);
   ...
   */
  addRLQuery(relatedTable: string, relatedField: string, operatorCondition: string, stopAtRelationship: boolean = false): any {
    if (!this._table) {
      throw new Error('Cannot execute RL query as Query object was not instantiated with a table name');
    }
    
    // Import dynamically to avoid circular dependency
    const { RLQuery } = require('./RLQuery');
    const rlQuery = new RLQuery(this._table, relatedTable, relatedField, operatorCondition, stopAtRelationship);
