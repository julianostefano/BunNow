...
export class SSEManager extends EventEmitter {
  ...
  createElysiaRoutes(): Elysia {
    return new Elysia()
      // Main SSE endpoint
      .get('/events/stream', async (context) => {
      ...
      .get('/events/tasks', async (context) => {
        return this.handleSSEConnection(context, ['tasks.progress', 'tasks.updates']);
      })

      .get('/events/system', async (context) => {
        return this.handleSSEConnection(context, ['system.health', 'system.metrics']);
      })
      ...
      .get('/events/performance', async (context) => {
        return this.handleSSEConnection(context, ['performance.alerts', 'system.metrics']);
      })
  ...
  private async handleSSEConnection(
    ...
    if (this.streams.size >= this.options.maxStreams) {
      set.status = 503;
      return new Response('Service Unavailable: Too many active streams', { status: 503 });
    }
    ...
    if (ipConnCount >= this.options.rateLimits.connectionsPerIP) {
      set.status = 429;
      return new Response('Too Many Requests: Connection limit per IP exceeded', { status: 429 });
    }

    // Parse parameters
    const channelsParam = query.channels || '';
    const requestedChannels = channelsParam ? channelsParam.split(',').map((c: string) => c.trim()) : [];
    const channels = defaultChannels || requestedChannels.filter(c => this.isValidChannel(c));
    ...
    const stream = new ReadableStream({
      start: (controller) => {
        ...
        console.log(`SSE stream connected: ${streamId} (channels: ${channels.join(', ')})`);
  ...
  broadcast(notification: Notification): void {
    ...
    for (const streamId of targetStreams) {
      ...
      if (streamInfo && this.shouldReceiveNotification(streamInfo, notification)) {
        if (this.sendEventToStream(streamId, {
          type: 'notification',
          data: notification,
          id: notification.id,
  ...
  private sendEventToStream(streamId: string, event: any): boolean {
    ...
    try {
      ...
      if (encodedData.length > this.options.maxEventSize) {
        console.warn(`Event too large for stream ${streamId}: ${encodedData.length} bytes`);
        return false;
      }
   ...
   */
  private shouldReceiveNotification(streamInfo: SSEStreamInfo, notification: Notification): boolean {
  ...
  private isValidChannel(channel: string): boolean {
    return this.SYSTEM_CHANNELS.includes(channel) ||
    ...
    return {
      ...
      performance: {
        totalEvents: Array.from(this.streams.values()).reduce(
          (sum, info) => sum + info.eventCount, 0
        ),
        avgEventsPerStream: this.streams.size > 0 
