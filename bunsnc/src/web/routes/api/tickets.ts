...
const app = new Elysia({ prefix: '/api/tickets' })
  ...
  .get('/stats/unified', async ({ query, enhancedMetricsService, contractualViolationService }) => {
    try {
      const startDate = query.start_date ? new Date(query.start_date) : new Date(Date.now() - 7 * 24 * 60 * 60 * 1000);
      const endDate = query.end_date ? new Date(query.end_date) : new Date();

      // Generate SLA metrics for all ticket types
      const slaMetrics = await enhancedMetricsService.generateSLAMetrics(startDate, endDate);

      // Generate violation statistics
      const violationStats = await contractualViolationService.generateViolationStatistics(startDate, endDate);

      // Calculate totals across all ticket types
      const totalMetrics = {
        total_tickets: slaMetrics.reduce((sum, m) => sum + m.total_tickets, 0),
        compliant_tickets: slaMetrics.reduce((sum, m) => sum + m.compliant_tickets, 0),
        breached_tickets: slaMetrics.reduce((sum, m) => sum + m.breached_tickets, 0),
        total_penalty_percentage: Math.round(slaMetrics.reduce((sum, m) => sum + m.total_penalty_percentage, 0) * 100) / 100,
      ...
      for (const metric of slaMetrics) {
        if (metric.average_response_time > 0) {
          totalResponseTime += metric.average_response_time * metric.total_tickets;
          responseTicketCount += metric.total_tickets;
        }
        if (metric.average_resolution_time > 0) {
          totalResolutionTime += metric.average_resolution_time * metric.total_tickets;
          resolutionTicketCount += metric.total_tickets;
        }
      }

      totalMetrics.average_response_time = responseTicketCount > 0
        ? Math.round((totalResponseTime / responseTicketCount) * 100) / 100
        : 0;
      totalMetrics.average_resolution_time = resolutionTicketCount > 0
        ? Math.round((totalResolutionTime / resolutionTicketCount) * 100) / 100
        : 0;

      const compliancePercentage = totalMetrics.total_tickets > 0
        ? Math.round((totalMetrics.compliant_tickets / totalMetrics.total_tickets) * 10000) / 100
        : 0;
      ...
      return {
        ...
        data: {
          ...
          summary: {
            ...
            highest_compliance_type: slaMetrics.length > 0
              ? slaMetrics.reduce((max, current) => current.compliance_percentage > max.compliance_percentage ? current : max).ticket_type
              : null,
            lowest_compliance_type: slaMetrics.length > 0
              ? slaMetrics.reduce((min, current) => current.compliance_percentage < min.compliance_percentage ? current : min).ticket_type
              : null,
      ...
      logger.error(' [TicketsAPI] Error generating unified statistics:', error);
      return {
        success: false,
  ...
  .get('/stats/by-groups', async ({ query }) => {
    try {
      ...
      const ticketTypes = [
        { type: TicketType.INCIDENT, collection: 'sn_incidents_collection', path: 'data.incident' },
        { type: TicketType.CTASK, collection: 'sn_ctasks_collection', path: 'data.change_task' },
        { type: TicketType.SCTASK, collection: 'sn_sctasks_collection', path: 'data.sc_task' }
      ];
      ...
      for (const { type, collection, path } of ticketTypes) {
        ...
        for (const ticket of tickets) {
          const ticketData = path.split('.').reduce((obj, key) => obj?.[key], ticket);
      ...
      return {
        ...
        data: {
          ...
          summary: {
            most_active_group: groupStatsList.length > 0 ? groupStatsList[0] : null,
            total_tickets_across_groups: groupStatsList.reduce((sum: number, group: any) => sum + group.total_tickets, 0)
          }
        },
        timestamp: new Date().toISOString()
      };
  ...
  .get('/violation/validate/:ticket_id', async ({ params, query, contractualViolationService }) => {
    try {
      const ticketType = (query.ticket_type as string)?.toLowerCase() as TicketType || TicketType.INCIDENT;
      ...
      logger.error(` [TicketsAPI] Error validating ticket ${params.ticket_id}:`, error);
      return {
        success: false,
        error: error instanceof Error ? error.message : String(error),
        timestamp: new Date().toISOString()
  ...
  .get('/violation/report', async ({ query, contractualViolationService }) => {
    try {
      const startDate = query.start_date ? new Date(query.start_date) : new Date(Date.now() - 30 * 24 * 60 * 60 * 1000);
      const endDate = query.end_date ? new Date(query.end_date) : new Date();
      ...
      return {
        ...
        data: {
          ...
          recommendations: this.generateViolationRecommendations(violationStats)
        },
        timestamp: new Date().toISOString()
      };
  ...
  .get('/health', async ({ contractualSLAService, enhancedMetricsService, contractualViolationService }) => {
    try {
      const slaHealthy = await contractualSLAService.healthCheck();
      const violationHealthy = await contractualViolationService.healthCheck();
...
function generateViolationRecommendations(stats: any): string[] {
  ...
  if (stats.violation_rate_percentage > 10) {
    recommendations.push('High violation rate detected. Review SLA configurations and group assignments.');
  }

  if (stats.total_financial_impact > 5) {
    recommendations.push('Significant financial impact from violations. Prioritize compliance improvements.');
  }

  if (Object.keys(stats.violations_by_ticket_type).length > 0) {
    const highestViolationType = Object.entries(stats.violations_by_ticket_type)
      .reduce((max, [type, count]) => (count as number) > max.count ? { type, count: count as number } : max, { type: '', count: 0 });

    if (highestViolationType.count > 0) {
      recommendations.push(`Focus on ${highestViolationType.type} tickets - highest violation count (${highestViolationType.count}).`);
    }
  }

  if (recommendations.length === 0) {
    recommendations.push('Good compliance levels maintained. Continue monitoring trends.');
  }
