...
const app = new Elysia({ prefix: '/api/sla-metrics' })
  ...
  .get('/config/:ticket_type', async ({ params, contractualSLAService }) => {
    try {
      ...
      logger.error(` [SLAMetrics] Error getting SLA config for ${params.ticket_type}:`, error);
      return {
        success: false,
        error: error instanceof Error ? error.message : String(error),
        timestamp: new Date().toISOString()
  ...
  .get('/ticket/:ticket_id/sla', async ({ params, query, enhancedMetricsService }) => {
    try {
      ...
      const slaStatus = await enhancedMetricsService.calculateTicketSLA(params.ticket_id, ticketType);
      ...
      logger.error(` [SLAMetrics] Error calculating SLA for ticket ${params.ticket_id}:`, error);
      return {
        success: false,
        error: error instanceof Error ? error.message : String(error),
        timestamp: new Date().toISOString()
  ...
  .get('/metrics', async ({ query, enhancedMetricsService }) => {
    try {
      const startDate = query.start_date ? new Date(query.start_date) : new Date(Date.now() - 7 * 24 * 60 * 60 * 1000); // Default: 7 days ago
      const endDate = query.end_date ? new Date(query.end_date) : new Date(); // Default: now
      const ticketType = query.ticket_type?.toLowerCase() as TicketType | undefined;
      ...
      const metrics = await enhancedMetricsService.generateSLAMetrics(startDate, endDate, ticketType);
      ...
      return {
        ...
        data: {
          ...
          summary: {
            ...
            overall_compliance: metrics.length > 0
              ? Math.round((metrics.reduce((sum, m) => sum + m.compliance_percentage, 0) / metrics.length) * 100) / 100
              : 0,
            total_penalties: Math.round(metrics.reduce((sum, m) => sum + m.total_penalty_percentage, 0) * 100) / 100
          }
        },
        timestamp: new Date().toISOString()
      };
  ...
  .get('/dashboard', async ({ query, enhancedMetricsService, contractualViolationService }) => {
    try {
      const startDate = query.start_date ? new Date(query.start_date) : new Date(Date.now() - 30 * 24 * 60 * 60 * 1000); // Default: 30 days ago
      const endDate = query.end_date ? new Date(query.end_date) : new Date(); // Default: now

      const dashboardData = await enhancedMetricsService.getDashboardData(startDate, endDate);
  ...
  .get('/compliance/summary', async ({ query, enhancedMetricsService }) => {
    try {
      ...
      const metrics = await enhancedMetricsService.generateSLAMetrics(startDate, endDate);
      ...
      summary.overall = {
        ...
        compliance_percentage: totalTickets > 0 ? Math.round((totalCompliant / totalTickets) * 10000) / 100 : 0,
      ...
      logger.error(' [SLAMetrics] Error generating compliance summary:', error);
      return {
        success: false,
        error: error instanceof Error ? error.message : String(error),
        timestamp: new Date().toISOString()
  ...
  .get('/penalties/report', async ({ query, enhancedMetricsService }) => {
    try {
      ...
      for (const metric of metrics) {
        ...
        for (const priorityMetric of metric.metrics_by_priority) {
          ...
          const existing = penaltyReport.penalties_by_priority[priorityMetric.priority];
      ...
      penaltyReport.total_penalty_percentage = Math.round(totalPenalty * 100) / 100;
