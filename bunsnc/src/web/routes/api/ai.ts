...
export const aiRoutes = new Elysia({ prefix: "/api/ai" })
  ...
  .post(
    ...
    },
    {
      ...
      detail: {
        ...
        description:
          "Analyzes a ServiceNow ticket to predict priority, category, assignment group, and provide resolution recommendations based on similar tickets",
        tags: ["AI", "Tickets"],
      },
    },
  )
  ...
  .get(
    "/tickets/analyze/config",
    async ({ ticketAnalysis }) => {
      try {
    ...
    },
    {
      detail: {
        ...
        description:
          "Returns current configuration for ticket analysis AI service",
  ...
  .put(
    ...
    },
    {
      ...
      detail: {
        ...
        description:
          "Updates configuration parameters for ticket analysis AI service",
        tags: ["AI", "Configuration"],
      },
    },
  )
  ...
  .post(
    "/documents/process",
    async ({ body, documentIntelligence, set }) => {
      try {
        logger.info(
          `📄 [AI-API] Processing document: ${(body as any).filename}`,
        );
    ...
    },
    {
      ...
      detail: {
        ...
        description:
          "Processes a document using Apache Tika for content extraction, NER for entity recognition, and classification for categorization",
        tags: ["AI", "Documents"],
      },
    },
  )
  ...
  .post(
    ...
    },
    {
      ...
      detail: {
        ...
        description:
          "Performs hybrid semantic search across technical documents with neural reranking for improved relevance",
        tags: ["AI", "Search"],
      },
    },
  )
  ...
  .get(
    ...
    },
    {
      detail: {
        ...
        description:
          "Returns current configuration for document intelligence AI service",
        tags: ["AI", "Configuration"],
      },
    },
  )
  ...
  .put(
    ...
    },
    {
      ...
      detail: {
        ...
        description:
          "Updates configuration parameters for document intelligence AI service",
        tags: ["AI", "Configuration"],
      },
    },
  )
  ...
  .get(
    "/health",
    async ({ ticketAnalysis, documentIntelligence, set }) => {
      try {
        const services = [
  ...
  .get(
    ...
    async ({ set }) => {
      try {
        ...
        const { EmbeddingClient } = await import(
        ...
        const { OpenSearchClient } = await import(
          "../../../clients/OpenSearchClient"
        );
        ...
        const serviceStatus = {
          tika: {
            status:
              statusChecks[0].status === "fulfilled" && statusChecks[0].value
                ? "healthy"
                : "unhealthy",
          },
          embedding: {
            status:
              statusChecks[1].status === "fulfilled" && statusChecks[1].value
                ? "healthy"
                : "unhealthy",
          },
          rerank: {
            status:
              statusChecks[2].status === "fulfilled" && statusChecks[2].value
                ? "healthy"
                : "unhealthy",
          },
          llm: {
            status:
              statusChecks[3].status === "fulfilled" && statusChecks[3].value
                ? "healthy"
                : "unhealthy",
          },
          opensearch: {
            status:
              statusChecks[4].status === "fulfilled" && statusChecks[4].value
                ? "healthy"
                : "unhealthy",
          },
        };

        const healthyCount = Object.values(serviceStatus).filter(
          (s) => s.status === "healthy",
        ).length;
        const overallStatus =
          healthyCount === Object.keys(serviceStatus).length
            ? "all_healthy"
            : healthyCount > 0
              ? "partially_healthy"
              : "all_unhealthy";
    ...
    },
    {
      detail: {
        ...
        description:
          "Returns detailed health status of all underlying AI services (Tika, OpenSearch, Embedding, Rerank, LLM)",
        tags: ["AI", "Health"],
      },
    },
  );
