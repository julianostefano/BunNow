...
const app = new Elysia({ prefix: '/api/incidents' })
  ...
  .get('/stats/summary', async ({ enhancedMetricsService, contractualViolationService }) => {
    try {
      ...
      resolvedTodayGr.addQuery('resolved_at', '>=', today.toISOString().split('T')[0] + ' 00:00:00');
  ...
  .get('/trends/hourly', async ({ query }) => {
    try {
      ...
      const gr = client.getGlideRecord('incident');
      gr.addQuery('sys_created_on', '>=', startDate.toISOString().split('T')[0] + ' 00:00:00');
      gr.query();
