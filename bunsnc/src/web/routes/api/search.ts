...
const IntelligentSearchRequestSchema = t.Object({
  ...
  targets: t.Optional(t.Array(t.Union([
    t.Literal('documents'),
    t.Literal('tickets'),
...
export class IntelligentSearchController {
  ...
  async performIntelligentSearch(request: any): Promise<any> {
    ...
    try {
      ...
      if (request.enable_reranking !== false && combinedResults.length > 1) {
        finalResults = await this.applyReranking(request.query, combinedResults, request.max_results || 20);
      } else {
      ...
      const response = {
        ...
        metadata: {
          ...
          reranking_applied: request.enable_reranking !== false && combinedResults.length > 1,
  ...
  private async searchDocuments(request: any, embedding: number[]): Promise<any[]> {
    try {
  ...
  private async searchTickets(request: any, embedding: number[]): Promise<any[]> {
    try {
      const searchType = request.search_type || 'hybrid';
      let results: any[] = [];
      ...
      return results.map(result => ({
        ...
        metadata: {
          ...
          assignment_group: result._source?.assignment_group || result.assignment_group,
  ...
  private async searchServiceNowTickets(request: any): Promise<any[]> {
    try {
      ...
      const searchPromises = tables.map(table =>
        this.serviceNow.makeRequest('GET', `/${table}`, {
          sysparm_query: `short_descriptionLIKE${request.query}^ORdescriptionLIKE${request.query}`,
          sysparm_limit: Math.ceil((request.max_results || 20) / tables.length),
          sysparm_fields: 'sys_id,number,short_description,description,state,priority,category,assignment_group,sys_created_on'
        }).catch(() => ({ result: [] }))
      );
  ...
  private async searchKnowledgeBase(request: any, embedding: number[]): Promise<any[]> {
    try {
  ...
  private combineResults(searchResults: any[][], query: string): any[] {
    ...
    searchResults.flat().forEach(result => {
      ...
      if (existingResult) {
        ...
        existingResult.relevance_score = Math.min(1.0, existingResult.relevance_score * 1.2);
  ...
  private async applyReranking(query: string, results: any[], maxResults: number): Promise<any[]> {
    try {
      ...
      return rerankResponse.results.map(rerankResult => {
        const originalResult = results.find(r => r.id === rerankResult.document.id);
        if (originalResult) {
          return {
            ...originalResult,
            rerank_score: rerankResult.relevance_score,
            final_score: (originalResult.relevance_score + rerankResult.relevance_score) / 2
          };
        }
        return null;
...
export const searchRoutes = new Elysia({ prefix: '/api/search' })
  ...
  .post('/intelligent', async ({ body, intelligentSearch, set }) => {
    try {
    ...
    detail: {
      ...
      description: 'Performs intelligent search across documents, tickets, and knowledge base using semantic, hybrid, or neural sparse search with optional reranking',
      tags: ['Search', 'AI']
    }
  })

  .get('/suggestions', async ({ query: { q, limit = 10 }, intelligentSearch }) => {
    try {
      if (!q || q.length < 2) {
        return {
          success: true,
    ...
    detail: {
      ...
      description: 'Returns intelligent search suggestions based on partial query',
      tags: ['Search']
    }
  })
  ...
  .get('/filters', async () => {
    try {
      ...
      const filterOptions = {
        document_types: [
          'Documentation', 'SQL Script', 'Code Documentation',
          'Process Guide', 'Troubleshooting', 'Technical Document'
        ],
        technology_stack: [
          'Oracle', 'PostgreSQL', 'AWS', 'Java', 'Python',
          'JavaScript', 'Docker', 'Kubernetes', 'Linux'
        ],
        categories: [
        ...
        search_types: [
          'semantic', 'hybrid', 'neural_sparse', 'bm25', 'all'
        ],
        targets: [
          'documents', 'tickets', 'knowledge_base', 'all'
        ]
      };
    ...
    detail: {
      ...
      description: 'Returns all available filter options for intelligent search',
      tags: ['Search']
    }
  });