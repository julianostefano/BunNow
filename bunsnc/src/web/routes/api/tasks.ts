...
import { TaskType, TaskPriority, TaskStatus } from '../../../background/TaskQueue';
...
const app = new Elysia({ prefix: '/api/v1/tasks' })
  ...
  .post('/:id/cancel', async ({ params, body }) => {
    try {
      if (!taskManager) {
        return {
          success: false,
  ...
  .post('/scheduled/:id/enable', async ({ params, body }) => {
    try {
      if (!taskManager) {
        return {
          success: false,
  ...
  .post('/export/parquet', async ({ body }) => {
    try {
      if (!taskManager) {
        return {
          success: false,
  ...
  .post('/pipeline/execute', async ({ body }) => {
    try {
      if (!taskManager) {
        return {
          success: false,
