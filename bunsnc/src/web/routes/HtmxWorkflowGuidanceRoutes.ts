...
export const workflowGuidanceRoutes = new Elysia({ prefix: '/workflow' })
  ...
  .post('/create', async ({ body, html }) => {
    try {
      const { ticketId, workflowType, priority = 'medium', description = '' } = body as any;
      ...
      try {
        const table = ticketId.startsWith('INC') ? 'incident' :
                     ticketId.startsWith('CHG') ? 'change_request' :
                     ticketId.startsWith('SCTASK') ? 'sc_task' : 'incident';

        const response = await serviceNowClient.makeRequest('GET', `/${table}`, {
      ...
      const steps = await generateWorkflowSteps(workflowType, description, ticketDetails, priority);
      ...
      logger.info(`Created workflow session ${sessionId} for ticket ${ticketId}`);
      ...
      return html(`<div class="error">Failed to create workflow: ${error}</div>`);
    }
  })
  ...
  // Start/Complete Step
  .post('/step/:sessionId/:stepIndex/:action', async ({ params, body, html }) => {
...
function renderWorkflowStep(session: WorkflowSession, stepIndex: number): string {
    ...
    incident_resolution: [
      {
        ...
        description: 'Gather information about the incident and assess severity',
        action_type: 'check',
        estimated_time: '5-10 min',
