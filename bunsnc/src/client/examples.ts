...
export async function batchOperationsExample() {
  ...
  try {
    ...
    const monitoringPromises = taskIds.map(taskId => 
      client.waitForTaskCompletion(taskId, {
    ...
    console.log(`${successfulTasks.length}/${taskIds.length} tasks completed successfully`);
      ...
      try {
        while (true) {
          ...
          if (currentProgress !== lastProgress) {
            ...
            const estimatedTotal = currentProgress > 0 ? elapsed / currentProgress * 100 : 0;
