...
export interface IServiceNowClient {
  ...
  listAttachments(table: string, tableSysId: string): Promise<ServiceNowRecord[]>;
...
export class ServiceNowClient implements IServiceNowClient {
   ...
   */
  GlideRecord(table: string, batchSize: number = 500, rewindable: boolean = true): GlideRecord {
  ...
  async query(options: QueryOptions): Promise<ServiceNowRecord[]> {
    ...
    try {
      ...
      if (this.cacheEnabled && options.query) {
        ...
        if (cached) {
          ...
          performanceMonitor.recordMetric({
            ...
            timestamp: Date.now(),
            context: 'ServiceNowClient',
            tags: { type: 'query', table: options.table, clientId: this.clientId }
          });
      ...
      const queryDuration = performanceMonitor.endTimer(`query_${options.table}`);
      ...
      performanceMonitor.recordMetric({
        ...
        timestamp: Date.now(),
        context: 'ServiceNowClient',
        tags: { table: options.table, cached: 'false', clientId: this.clientId }
      });
  ...
  async uploadAttachment(
    ...
    const operation = logger.operation('upload_attachment', table, tableSysId, {
      ...
      fileName,
      contentType,
      fileSize: file instanceof Buffer ? file.length : file instanceof Blob ? file.size : 'unknown'
    });

    try {
      const result = await this.attachment.upload(fileName, table, tableSysId, file, contentType);
  ...
  async downloadAttachment(sysId: string): Promise<Response> {
    const operation = logger.operation('download_attachment', undefined, sysId, {
      clientId: this.clientId
    });
   ...
   */
  async listAttachments(table: string, tableSysId: string): Promise<ServiceNowRecord[]> {
  ...
  async testConnection(): Promise<boolean> {
    try {
      const response = await fetch(`${this.instance}/api/now/table/sys_properties?sysparm_limit=1`, {
        method: 'GET',
        headers: {
   ...
   */
  async executeSequence(operations: Array<{
    ...
    const operation = logger.operation('execute_sequence', 'batch', undefined, {
      ...
      operations: operations.map(op => ({ method: op.method, table: op.table }))
    });
   ...
   */
  async getAttachmentAsText(sysId: string, encoding: string = 'utf-8'): Promise<string> {
    const operation = logger.operation('get_attachment_text', undefined, sysId, {
      clientId: this.clientId,
      encoding
    });
  ...
  async getAttachmentAsBlob(sysId: string): Promise<Blob> {
    const operation = logger.operation('get_attachment_blob', undefined, sysId, {
      clientId: this.clientId
    });
   ...
   */
  async getAttachmentWithStats(sysId: string): Promise<ServiceNowRecord & { fileExists: boolean; accessible: boolean }> {
    const operation = logger.operation('get_attachment_stats', undefined, sysId, {
      clientId: this.clientId
    });
   ...
   */
  async bulkDeleteAttachments(sysIds: string[]): Promise<{ deleted: number; errors: Array<{ sysId: string; error: string }> }> {
  ...
  enableCache(enabled: boolean): void {
    ...
    logger.info(`Cache ${enabled ? 'enabled' : 'disabled'}`, 'ServiceNowClient', {
      clientId: this.clientId
    });
  }
  ...
  getSystemHealth(): any {
    ...
    const healthScore = this.calculateHealthScore(avgResponseTime, errorRate, cacheStats.hitRate);
  ...
  private calculateHealthScore(avgResponseTime: number, errorRate: number, cacheHitRate: number): number {
    let score = 1.0;
