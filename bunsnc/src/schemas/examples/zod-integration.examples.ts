...
export function exampleCreateIncident() {
  ...
  const incidentData = {
    ...
    description:
      "Multiple users reporting inability to access email service. Error message indicates server connection timeout.",
  ...
  try {
    ...
    if (validationResult.success) {
      console.log(
        " Incident validation successful:",
        validationResult.data?.short_description,
      );
    } else {
...
export function exampleCreateChangeTask() {
  ...
  const changeTaskData = {
    ...
    description:
      "Add new user profile fields to support enhanced user management features",
  ...
  try {
    ...
    if (validationResult.success) {
      console.log(
        " Change task validation successful:",
        validationResult.data?.short_description,
      );
    } else {
...
export function exampleCreateServiceTask() {
  ...
  const serviceTaskData = {
    ...
    description:
      "Configure and deploy new laptop with standard software package",
  ...
  try {
    ...
    if (validationResult.success) {
      console.log(
        " Service task validation successful:",
        validationResult.data?.short_description,
      );
    } else {
...
export function exampleSLAValidation() {
  ...
  try {
    ...
    if (validation.success) {
      ...
      console.log(
        `Resolution time: ${resolutionHours.toFixed(1)} hours (Target: 4 hours for Critical)`,
      );
    } else {
...
export function exampleServiceRequestValidation() {
  ...
  const serviceRequestData = {
    ...
    variables: {
      ...
      business_justification:
        "Required for marketing campaign design and multimedia content creation for Q1 2025 product launch",
      estimated_cost: 7500, // High-value item requiring approval
    },
    quantity: 5,
    business_justification:
      "Team needs design software for upcoming marketing campaigns",
  ...
  try {
    const validation =
      ServiceRequestValidationSchema.safeParse(serviceRequestData);
    ...
    if (validation.success) {
      ...
      console.log(
        "Estimated cost:",
        `$${serviceRequestData.variables.estimated_cost}`,
      );
      console.log(
        "Manager approval:",
        serviceRequestData.manager_approval?.approved_at
          ? " Approved"
          : "⏳ Pending",
      );
    } else {
...
export function exampleIncidentEscalation() {
  ...
  try {
    const validation =
      IncidentEscalationValidationSchema.safeParse(escalationData);

    if (validation.success) {
      console.log(" Incident escalation level is appropriate");
      const ageHours =
        (currentTime.getTime() - createdTime.getTime()) / (1000 * 60 * 60);
      console.log(
        `Incident age: ${ageHours.toFixed(1)} hours, Escalation level: ${escalationData.escalation}`,
      );
    } else {
...
export function exampleErrorHandling() {
  ...
  const incompleteValidation = safeValidateSchema(
    "incident-creation",
    incompleteData,
  );
