...
export const IncidentStateSchema = z.enum(
  ["1", "2", "3", "6", "7", "8", "18"],
  {
    errorMap: () => ({
      message:
        "Invalid incident state. Must be: 1(New), 2(In Progress), 3(On Hold), 6(Resolved), 7(Closed), 8(Cancelled), 18(Assigned)",
    }),
  },
);
 ...
 */
export const ChangeTaskStateSchema = z.enum(
  ["-5", "1", "2", "3", "4", "7", "8"],
  {
    errorMap: () => ({
      message:
        "Invalid change task state. Must be: -5(Pending), 1(Open), 2(Assigned), 3(In Progress), 4(Closed Complete), 7(Closed Skipped), 8(Closed Incomplete)",
    }),
  },
);
...
export const ServiceTaskStateSchema = z.enum(["-5", "1", "2", "3", "4", "7"], {
  errorMap: () => ({
    message:
      "Invalid service task state. Must be: -5(Pending), 1(Open), 2(Assigned), 3(In Progress), 4(Closed Complete), 7(Closed Skipped)",
  }),
});
 ...
 */
export const EncodedQuerySchema = z.string().refine(
  (query) => {
    ...
    return (
      query.includes("=") ||
      query.includes("!=") ||
      validConnectors.some((conn) => query.includes(conn))
    );
  },
  {
...
export const ServiceNowBatchRequestSchema = z.object({
  ...
  rest_requests: z
    ...
    .max(100, "Maximum 100 requests allowed"),
});
 ...
 */
export const ServiceNowAuthConfigSchema = z
  .object({
...
export type ServiceNowBusinessService = z.infer<
  typeof ServiceNowBusinessServiceSchema
>;
...
export type ServiceNowTableRequest = z.infer<
  typeof ServiceNowTableRequestSchema
>;
