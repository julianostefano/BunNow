 ...
 */
export const SLAValidationSchema = z.object({
  ...
  const resolutionTimeHours = (resolvedDate.getTime() - createdDate.getTime()) / (1000 * 60 * 60);
  ...
  message: 'SLA breach detected: Resolution time exceeds target for priority level',
  path: ['sla_compliance']
});
 ...
 */
export const ChangeApprovalValidationSchema = z.object({
  ...
  if (['high', 'very_high'].includes(data.risk)) {
    const cabApproval = data.approvals.find(a => a.approver === 'cab_approval_sys_id');
    if (!cabApproval) {
      ctx.addIssue({
  ...
  if (data.change_type === 'emergency') {
    ...
    const createdDate = new Date(data.created_on);
    const timeDiffHours = (implementationDate.getTime() - createdDate.getTime()) / (1000 * 60 * 60);
    
    if (timeDiffHours > 24) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: 'Emergency changes should be implemented within 24 hours of creation',
  ...
  if (data.change_type === 'standard' && data.approvals.length > 0) {
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
      message: 'Standard changes are pre-approved and should not require individual approvals',
      path: ['approvals']
    });
  }
});
...
export const ServiceRequestValidationSchema = z.object({
  ...
  if (data.variables?.category === 'software') {
    const requiredSoftwareFields = ['software_name', 'version', 'license_type', 'business_justification'];
  ...
  if (data.requested_for === data.requester) {
    ...
    if (!data.business_justification || data.business_justification.length < 20) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: 'Self-service requests require detailed business justification (min 20 characters)',
 ...
 */
export const IncidentEscalationValidationSchema = z.object({
  ...
  const expectedEscalationLevel = Math.min(Math.floor(ageHours / rule.hours), rule.maxLevel);
  
  // VIP incidents escalate faster
  const vipMultiplier = data.vip ? 0.5 : 1;
  const adjustedExpectedLevel = Math.min(Math.floor(ageHours / (rule.hours * vipMultiplier)), rule.maxLevel);
 ...
 */
export const CIRelationshipValidationSchema = z.object({
  parent: SysIdSchema,
  child: SysIdSchema,
  type: z.enum(['depends_on', 'uses', 'connects_to', 'runs_on', 'installed_on']),
  ...
  if (directionalTypes.includes(data.type) && data.direction === 'bidirectional') {
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
 ...
 */
export const KnowledgeArticleValidationSchema = z.object({
  ...
  approval_state: z.enum(['draft', 'review', 'approved', 'published', 'retired']),
  ...
  if (data.confidentiality !== 'public' && data.target_audience === 'customer') {
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
 ...
 */
export const AssetLifecycleValidationSchema = z.object({
  asset_tag: z.string().min(1),
  serial_number: z.string().min(1),
  state: z.enum(['on_order', 'in_stock', 'in_use', 'in_maintenance', 'retired', 'disposed']),
  assigned_to: SysIdSchema.optional(),
  location: SysIdSchema.optional(),
  cost: z.number().min(0),
  purchase_date: ServiceNowDateTimeSchema,
  warranty_expiration: ServiceNowDateTimeSchema.optional(),
  depreciation_schedule: z.enum(['straight_line', 'declining_balance', 'none']).default('straight_line'),
...
export type ChangeApprovalValidation = z.infer<typeof ChangeApprovalValidationSchema>;
export type ServiceRequestValidation = z.infer<typeof ServiceRequestValidationSchema>;
export type IncidentEscalationValidation = z.infer<typeof IncidentEscalationValidationSchema>;
export type CIRelationshipValidation = z.infer<typeof CIRelationshipValidationSchema>;
export type KnowledgeArticleValidation = z.infer<typeof KnowledgeArticleValidationSchema>;
export type AssetLifecycleValidation = z.infer<typeof AssetLifecycleValidationSchema>;