...
export const IncidentSchema = BaseTicketSchema.extend({
  ...
  // Reassignment count
  reassignment_count: z.string().transform(val => parseInt(val) || 0).optional(),
  reopen_count: z.string().transform(val => parseInt(val) || 0).optional()
});
 ...
 */
export const IncidentStateTransitionsSchema = z.object({
  from_state: IncidentStateSchema,
  to_state: IncidentStateSchema,
  action: z.enum(['assign', 'work', 'hold', 'resolve', 'close', 'cancel', 'reopen']),
...
export const IncidentQuerySchema = z.object({
  state: z.union([IncidentStateSchema, z.array(IncidentStateSchema)]).optional(),
...
export type IncidentStateTransition = z.infer<typeof IncidentStateTransitionsSchema>;