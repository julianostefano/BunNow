 ...
 */
export const HDFSPathSchema = z
  .string()
  .min(1, "HDFS path cannot be empty")
  .regex(/^\//, "HDFS path must start with /")
  .refine(
    (path) => !path.includes("//"),
    "HDFS path cannot contain double slashes",
  )
  .refine(
    (path) => !path.endsWith("/") || path === "/",
    "HDFS path cannot end with slash except root",
  );
...
export const ParquetSchemaSchema = z.object({
  ...
  compression: z
    .enum(["uncompressed", "snappy", "gzip", "lzo"])
    .default("snappy"),
...
export const ProcessingJobSchema = z.object({
  ...
  // Input/Output paths
  input_paths: z
    .array(HDFSPathSchema)
    .min(1, "At least one input path required"),
