...
function convertZodToTypeBox(def: any, config: TypeMappingConfig): TAnySchema {
  ...
  console.warn(
    `Unsupported Zod type: ${def.typeName}, falling back to t.Any()`,
  );
  return t.Any();
}
...
export function getConversionWarnings(zodSchema: z.ZodTypeAny): string[] {
  ...
  if (def.typeName === "ZodString" && def.checks) {
    for (const check of def.checks) {
      if (
        ["regex", "includes", "startsWith", "endsWith"].includes(check.kind)
      ) {
        warnings.push(
          `String check '${check.kind}' may have limited TypeBox support`,
        );
      }
    }
  }
  ...
  if (def.typeName === "ZodEffects") {
    warnings.push(
      "Refinements and transforms will be lost in TypeBox conversion",
    );
  }
